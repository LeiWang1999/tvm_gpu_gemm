@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  attr [IterVar(threadIdx.x: int32, (nullptr), "ThreadIndex", "threadIdx.x")] "thread_extent" = 16 {
    ...
    for (k.outer: int32, 0, 512) {
      for (ax0: int32, 0, 128) {
        for (ax1: int32, 0, 32) {
          A.shared_1: Buffer(A.shared, int8, [4096], [], scope="shared")[((ax0*32) + ax1)] = A[((((blockIdx.x*2097152) + (ax0*16384)) + (k.outer*32)) + ax1)]
        }
      }
      for (ax0_1: int32, 0, 128) {
        for (ax1_1: int32, 0, 32) {
          B.shared_1: Buffer(B.shared, int8, [4096], [], scope="shared")[((ax0_1*32) + ax1_1)] = B[((((blockIdx.y*2097152) + (ax0_1*16384)) + (k.outer*32)) + ax1_1)]
        }
      }
      for (k.inner: int32, 0, 32) {
        for (i.c: int32, 0, 8) {
          for (j.c: int32, 0, 8) {
            let cse_var_1: int32 = ((i.c*8) + j.c)
            C.local_1[cse_var_1] = (C.local_1[cse_var_1] + (cast(int32, A.shared_1[(((threadIdx.x*256) + (i.c*32)) + k.inner)])*cast(int32, B.shared_1[(((threadIdx.y*256) + (j.c*32)) + k.inner)])))
          }
        }
      }
    }
