@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(A.shared: Pointer(shared float16), float16, [268435456i64]), storage_scope = shared;
  allocate(A.shared.local: Pointer(local float16), float16, [268435456]), storage_scope = local;
  allocate(B.shared.local: Pointer(local float16), float16, [268435456]), storage_scope = local;
  allocate(C.local: Pointer(local float16), float16, [268435456]), storage_scope = local {
    ...
    for (ax0_1: int32, 0, 16384) {
      for (ax1_1: int32, 0, 16384) {
        ...
        A.shared.local_1: Buffer(A.shared.local, float16, [268435456], [], scope="local")[cse_var_2] = A.shared_1[cse_var_2]
      }
    }
    ...
    for (ax0_3: int32, 0, 16384) {
      for (ax1_3: int32, 0, 16384) {
        ...
        B.shared.local_1: Buffer(B.shared.local, float16, [268435456], [], scope="local")[cse_var_4] = A.shared_2[cse_var_4]
      }
    }
    for (ii.c: int32, 0, 16384) {
      for (jj.c: int32, 0, 16384) {
        C.local_1: Buffer(C.local, float16, [268435456], [], scope="local")[((ii.c*16384) + jj.c)] = 0f16
        for (k: int32, 0, 16384) {
    ...
    for (ii.outer: int32, 0, 128) {
      for (ii.inner: int32, 0, 128) {
        for (jj: int32, 0, 16384) {
          let cse_var_7: int32 = (((ii.outer*2097152) + (ii.inner*16384)) + jj)
