...
extern "C" __global__ void __launch_bounds__(256) default_function_kernel0(half* __restrict__ A, half* __restrict__ B, half* __restrict__ C) {
  half C_local[64];
  __shared__ uint2 A_shared[512];
  __shared__ uint2 B_shared[512];
  ...
  for (int k_outer = 0; k_outer < 1024; ++k_outer) {
    ...
    for (int ax1_outer = 0; ax1_outer < 2; ++ax1_outer) {
      A_shared[(((((int)threadIdx.y) * 32) + (ax1_outer * 16)) + ((int)threadIdx.x))] = *(uint2*)(A + (((((k_outer * 262144) + (((int)threadIdx.y) * 16384)) + (((int)blockIdx.y) * 128)) + (ax1_outer * 64)) + (((int)threadIdx.x) * 4)));
    }
    for (int ax1_outer1 = 0; ax1_outer1 < 2; ++ax1_outer1) {
      B_shared[(((((int)threadIdx.y) * 32) + (ax1_outer1 * 16)) + ((int)threadIdx.x))] = *(uint2*)(B + (((((k_outer * 262144) + (((int)threadIdx.y) * 16384)) + (((int)blockIdx.x) * 128)) + (ax1_outer1 * 64)) + (((int)threadIdx.x) * 4)));
    }
    __syncthreads();
    for (int k_inner = 0; k_inner < 16; ++k_inner) {
      *(uint2*)(A_shared_local + 0) = A_shared[((k_inner * 32) + ((int)threadIdx.y))];
      *(uint2*)(A_shared_local + 4) = A_shared[(((k_inner * 32) + ((int)threadIdx.y)) + 16)];
      *(uint2*)(B_shared_local + 0) = B_shared[((k_inner * 32) + ((int)threadIdx.x))];
      *(uint2*)(B_shared_local + 4) = B_shared[(((k_inner * 32) + ((int)threadIdx.x)) + 16)];
