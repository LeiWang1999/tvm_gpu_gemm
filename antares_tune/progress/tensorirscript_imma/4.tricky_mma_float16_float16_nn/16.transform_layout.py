...
def func(A: T.Buffer[(512, 512, 16, 16), "float16"], B: T.Buffer[(512, 512, 16, 16), "float16"], C: T.Buffer[(512, 512, 16, 16), "float16"]):
    ...
    A_shared_warp = T.alloc_buffer([512, 512, 32, 8], dtype="float16", scope="warp")
    B_shared = T.alloc_buffer([512, 512, 16, 16], dtype="float16", scope="shared")
    B_shared_warp = T.alloc_buffer([512, 512, 32, 8], dtype="float16", scope="warp")
                ...
                for ii_1 in T.thread_binding(1, thread="threadIdx.y"):
                    for jj_1 in T.thread_binding(4, thread="threadIdx.z"):
                        for ii_2_init, jj_2_init, i_init, j_init in T.grid(8, 4, 16, 16):
                            with T.block("B_init"):
                                vii = T.axis.spatial(512, ii_0 * 8 + ii_1 * 8 + ii_2_init)
                                vjj = T.axis.spatial(512, jj_0_0 * 256 + jj_0_1 * 16 + jj_1 * 4 + jj_2_init)
                            ...
                            for kk_1 in T.serial(2):
                                ...
                                for ii_2, jj_2, i, j, k in T.grid(8, 4, 16, 16, 16):
                                    with T.block("B_update"):
                                        ...
                                        T.reads(C_warp[vii, vjj, vi % 8 * 4 + vj % 8 // 2, vj // 8 * 4 + vi // 8 * 2 + vj % 2], A_shared_warp[vii, vkk, vi * 2 + vk // 8, vk % 8], B_shared_warp[vkk, vjj, vk * 2 + vj // 8, vj % 8])
                                        T.writes(C_warp[vii, vjj, vi % 8 * 4 + vj % 8 // 2, vj // 8 * 4 + vi // 8 * 2 + vj % 2])
                                        C_warp[vii, vjj, vi % 8 * 4 + vj % 8 // 2, vj // 8 * 4 + vi // 8 * 2 + vj % 2] = C_warp[vii, vjj, vi % 8 * 4 + vj % 8 // 2, vj // 8 * 4 + vi // 8 * 2 + vj % 2] + A_shared_warp[vii, vkk, vi * 2 + vk // 8, vk % 8] * B_shared_warp[vkk, vjj, vk * 2 + vj // 8, vj % 8]
