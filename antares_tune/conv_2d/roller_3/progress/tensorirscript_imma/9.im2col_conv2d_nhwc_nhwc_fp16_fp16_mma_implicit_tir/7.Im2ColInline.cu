    ...
    Conv_warp = alloc_buffer(float16[225792, 384])
     {
      ...
      for (ax0: int32, 0, 225792) {
        for (ax1: int32, 0, 1024) {
          block([225792, 1024], "data_im2col_shared") as [v0, v1] {
            ...
            data_im2col_shared[v0, v1] = @tir.if_then_else(((((0 <= ((1*floordiv(floormod(v0, 1764), 42)) + (1*floordiv(floordiv(v1, 1024), 1)))) && (((1*floordiv(floormod(v0, 1764), 42)) + (1*floordiv(floordiv(v1, 1024), 1))) < 42)) && (0 <= ((1*floormod(floormod(v0, 1764), 42)) + (1*floormod(floordiv(v1, 1024), 1))))) && (((1*floormod(floormod(v0, 1764), 42)) + (1*floormod(floordiv(v1, 1024), 1))) < 42)), A[floordiv(v0, 1764), (((1*floordiv(floormod(v0, 1764), 42)) + (1*floordiv(floordiv(v1, 1024), 1))) - 0), (((1*floormod(floormod(v0, 1764), 42)) + (1*floormod(floordiv(v1, 1024), 1))) - 0), floormod(v1, 1024)], 0f16, dtype=float16)
        }
      }
      ...
      for (x_1: int32, 0, 225792) {
        for (y_1: int32, 0, 384) {
          for (k: int32, 0, 1024) {
            block([225792, 384, tir.reduce_axis(0, 1024)], "Conv") as [v_x, v_y, v_k_1] {
              bind(v_x, x_1)
              bind(v_y, y_1)
              bind(v_k_1, k)
              tir.reads([data_im2col_shared_warp[v_x, v_k_1], weight_flatten_shared_warp[v_y, v_k_1]])
              tir.writes([Conv_warp[v_x, v_y]])
              with init() {
                Conv_warp[v_x, v_y] = 0f16
              }
              Conv_warp[v_x, v_y] = (Conv_warp[v_x, v_y] + (data_im2col_shared_warp[v_x, v_k_1]*weight_flatten_shared_warp[v_y, v_k_1]))
          }
        }
      }
