    ...
    for (k.outer: int32, 0, 1024) {
      for (ax1.outer: int32, 0, 2) {
        for (ax1.inner.inner: int32, 0, 4) {
          ...
          A.shared_1: Buffer(A.shared, float16, [2048], [], scope="shared")[((((threadIdx.y*128) + cse_var_2) + (threadIdx.x*4)) + ax1.inner.inner)] = A[((((((k.outer*262144) + (threadIdx.y*16384)) + (blockIdx.y*128)) + cse_var_2) + (threadIdx.x*4)) + ax1.inner.inner)]
        }
      }
      for (ax1.outer_1: int32, 0, 2) {
        for (ax1.inner.inner_1: int32, 0, 4) {
          let cse_var_3: int32 = (ax1.outer_1*64)
          B.shared_1: Buffer(B.shared, float16, [2048], [], scope="shared")[((((threadIdx.y*128) + cse_var_3) + (threadIdx.x*4)) + ax1.inner.inner_1)] = B[((((((k.outer*262144) + (threadIdx.y*16384)) + (blockIdx.x*128)) + cse_var_3) + (threadIdx.x*4)) + ax1.inner.inner_1)]
        }
      }
