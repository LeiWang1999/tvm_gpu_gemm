@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(A.shared.local: Pointer(local float16), float16, [65536]), storage_scope = local;
  allocate(B.shared.local: Pointer(local float16), float16, [65536]), storage_scope = local {
    for (ax0: int32, 0, 16384) {
      for (ax1: int32, 0, 4) {
        A.shared_1: Buffer(A.shared, float16, [65536], [], scope="shared")[((ax0*4) + ax1)] = A[(((((ax0*16384) + (blockIdx.y: int32*128)) + (vy: int32*64)) + (threadIdx.y: int32*4)) + ax1)]
      }
    }
    for (ax0_1: int32, 0, 16384) {
      for (ax1_1: int32, 0, 4) {
        let cse_var_1: int32 = ((ax0_1*4) + ax1_1)
        A.shared.local_1: Buffer(A.shared.local, float16, [65536], [], scope="local")[cse_var_1] = A.shared_1[cse_var_1]
      }
    }
    for (ax0_2: int32, 0, 16384) {
      for (ax1_2: int32, 0, 4) {
        A.shared_2: Buffer(A.shared, float16, [65536], [], scope="shared")[((ax0_2*4) + ax1_2)] = B[(((((ax0_2*16384) + (blockIdx.x: int32*128)) + (vx: int32*64)) + (threadIdx.x: int32*4)) + ax1_2)]
      }
    }
    for (ax0_3: int32, 0, 16384) {
      for (ax1_3: int32, 0, 4) {
        let cse_var_2: int32 = ((ax0_3*4) + ax1_3)
        B.shared.local_1: Buffer(B.shared.local, float16, [65536], [], scope="local")[cse_var_2] = A.shared_2[cse_var_2]
      }
    }
    attr [IterVar(blockIdx.y, (nullptr), "ThreadIndex", "blockIdx.y")] "thread_extent" = 128;
    allocate(C.local: Pointer(local float16), float16, [16]), storage_scope = local;
    ...
    attr [IterVar(threadIdx.x, (nullptr), "ThreadIndex", "threadIdx.x")] "thread_extent" = 16 {
      for (ii.c.init: int32, 0, 4) {
        for (jj.c.init: int32, 0, 4) {
          C.local_1: Buffer(C.local, float16, [16], [], scope="local", align=32)[((ii.c.init*4) + jj.c.init)] = 0f16
        }
      }
