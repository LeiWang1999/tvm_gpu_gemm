  ...
  attr [IterVar(threadIdx.z: int32, (nullptr), "ThreadIndex", "threadIdx.z")] "thread_extent" = 2 {
        ...
        for (ax0: int32, 0, 2) {
          for (ax2: int32, 0, 16) {
            for (ax3: int32, 0, 16) {
              ...
              A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, float16, [512], [], scope="wmma.matrix_a")[(((ax0*256) + cse_var_2) + ax3)] = A.shared_1[(((((threadIdx.y*2048) + (ax0*1024)) + (k1.inner*256)) + cse_var_2) + ax3)]
            }
          }
        }
        for (ax0_1: int32, 0, 4) {
          for (ax2_1: int32, 0, 16) {
            for (ax3_1: int32, 0, 16) {
              ...
              B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, float16, [1024], [], scope="wmma.matrix_b")[(((ax0_1*256) + cse_var_3) + ax3_1)] = B.shared_1[(((((threadIdx.z*4096) + (ax0_1*1024)) + (k1.inner*256)) + cse_var_3) + ax3_1)]
            }
          }
        }
        for (i.c: int32, 0, 2) {
          for (j.c: int32, 0, 4) {
            for (ii.c: int32, 0, 16) {
              for (jj.c: int32, 0, 16) {
                for (k2: int32, 0, 16) {
                  let cse_var_6: int32 = (j.c*256)
                  let cse_var_5: int32 = (ii.c*16)
                  let cse_var_4: int32 = ((((i.c*1024) + cse_var_6) + cse_var_5) + jj.c)
                  C.wmma.accumulator_1[cse_var_4] = (C.wmma.accumulator_1[cse_var_4] + (cast(float32, A.shared.wmma.matrix_a_1[(((i.c*256) + cse_var_5) + k2)])*cast(float32, B.shared.wmma.matrix_b_1[((cse_var_6 + (jj.c*16)) + k2)])))
                }
              }
            }
          }
    ...
    for (i.inner: int32, 0, 2) {
      for (j.inner: int32, 0, 4) {
        for (ii: int32, 0, 16) {
          for (jj: int32, 0, 16) {
            let cse_var_8: int32 = (j.inner*256)
            let cse_var_7: int32 = (ii*16)
            C[((((((((blockIdx.x*1048576) + (threadIdx.y*524288)) + (i.inner*262144)) + (blockIdx.y*2048)) + (threadIdx.z*1024)) + cse_var_8) + cse_var_7) + jj)] = C.wmma.accumulator_1[((((i.inner*1024) + cse_var_8) + cse_var_7) + jj)]
          }
        }
      }
    }
