...
def intrin_wmma_gemm(shape):
    ...
    B = te.placeholder((m, l), name="B", dtype="float16")
    k = te.reduce_axis((0, l), name="k")
    C = te.compute(
        (n, m),
        lambda ii, jj: te.sum(A[ii, k].astype(
            "float") * B[jj, k].astype("float"), axis=k),
        name="C",
    )
    BA = tvm.tir.decl_buffer(
...
def intrin_wmma_store_matrix(shape):
    n, m, l = shape
    A = te.placeholder((n, m), name="A", dtype="float")
    BA = tvm.tir.decl_buffer(
...
def test_tensor_core_matmal():
    log_path = "progress/tensorexpression_float16_tensorcore/1.tricky_tensorize_nt"
    wmma_m = 16
    wmma_n = 16
    wmma_k = 16
    m = 128
    ...
    B = te.placeholder((mm, ll, wmma_n, wmma_k), name="B", dtype="float16")
    k1 = te.reduce_axis((0, ll), name="k1")
    k2 = te.reduce_axis((0, wmma_k), name="k2")
    C = te.compute(
        (nn, mm, wmma_m, wmma_n),
        lambda i, j, ii, jj: te.sum(
            A[i, k1, ii, k2].astype("float") * B[j, k1, jj, k2].astype("float"), axis=[k1, k2]
        ),
        name="C",
    )
    ...
    b_np = np.random.uniform(size=(mm, ll, wmma_n, wmma_k)).astype(B.dtype)
