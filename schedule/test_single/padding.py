...
def func(A: T.Buffer[(16384, 16384), "int8"], B: T.Buffer[(16384, 16384), "int8"], C: T.Buffer[(16384, 16384), "int32"], PA: T.Buffer[16384, "int32"]):
    ...
    for i_0 in T.thread_binding(128, thread="blockIdx.y"):
        for j_0 in T.thread_binding(64, thread="blockIdx.x"):
            for i_1_0_init in T.thread_binding(2):
                for j_1_0_init in T.thread_binding(4):
                    for i_1_1_0_init, j_1_1_0_init in T.grid(4, 4):
                        with T.block("B_init_o"):
                            vi_o = T.axis.spatial(1024, i_0 * 8 + i_1_0_init * 4 + i_1_1_0_init)
                            vj_o = T.axis.spatial(1024, j_0 * 16 + j_1_0_init * 4 + j_1_1_0_init)
                            T.reads()
