    ...
    def main(a: T.handle, b: T.handle, c: T.handle):
        ...
        for i, j, k in T.grid(M, N, K):
            with T.block("B"):
                ...
                with T.init():
                    C[vi, vj] = 0.0
                C[vi, vj] = C[vi, vj] + A[vi, vk].astype("int32") * B[vk, vj].astype("int32")
...
A_shared_ty, A_shared_tx, A_shared_inner = sch.split(A_shared_fused, factors=[16, 16, 16])
...
sch.transform_layout(block_local_permutation_B, ("write", 0), B_local_permutation)

write_code(sch.mod["main"].script(), log_path, "transform_B_local_permutation.py")
...
write_code(sch.mod["main"].script(), log_path, "schedule_B_local_permutation.py")
...
block_local_C_j, block_local_C_vj = sch.split(block_local_C_j, factors=[None, 4])
