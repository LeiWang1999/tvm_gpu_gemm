...
warp_row_tiles = 8
warp_col_tiles = 4
double_buffer_stages = 2 # 1 is no double buffer 2 is double buffer enabled
chunk = 2
vec = 8
wmma_m = 16
wmma_n = 16
...
block_tricky_shared_local_B_loops = tricky_extract_cache(
    block_tricky_shared_local_B, wmma_k, wmma_n)
...
if double_buffer_stages == 2:
    sch.annotate(ko, ann_key="software_pipeline_stage", ann_val=[1])
    sch.annotate(ko, ann_key="software_pipeline_order", ann_val=[0, 1, 2])

sch.annotate(ko, ann_key="thread_rasterization", ann_val=[0, 1, 2])
