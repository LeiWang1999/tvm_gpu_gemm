...
mma_i_ty, mma_i= sch.split(mma_i, factors=[8, MMA_M // 8])
mma_j_tx, mma_j = sch.split(mma_j, factors=[4, MMA_N // 4])
sch.reorder(i, j, k, mma_i_ty, mma_j_tx, mma_k, mma_i, mma_j)
sch.bind(i, "blockIdx.y")
sch.bind(j, "blockIdx.x")
sch.bind(mma_i_ty, "threadIdx.y")
sch.bind(mma_j_tx, "threadIdx.x")
...
block_shared_A_outer, block_shared_A_ty, block_shared_A_tx, block_shared_A_vi = sch.split(block_shared_A_fused_i_j, factors=[None, 8, 4, vec])

sch.vectorize(block_shared_A_vi)
sch.bind(block_shared_A_ty, "threadIdx.y")
...
block_shared_B_outer, block_shared_B_ty, block_shared_B_tx, block_shared_B_vj = sch.split(block_shared_B_fused_i_j, factors=[None, 8, 4, vec])

sch.vectorize(block_shared_B_vj)
sch.bind(block_shared_B_ty, "threadIdx.y")
...
# block_cl_outer, block_cl_vi = sch.split(block_cl_fused_i_j, factors=[None, vec])
# sch.vectorize(block_cl_vi)

init_block = sch.decompose_reduction(block_b, k)
init_block_i, init_block_j = sch.get_loops(init_block)[-4:-2]
sch.bind(init_block_i, "threadIdx.y")
...
cuda_a = tvm.nd.array(np.arange(M * K).reshape((K, M)).astype("float32"), ctx)
