...
class MyModule:
    ...
    def main(a: T.handle, b: T.handle, c: T.handle):
        ...
        B = T.match_buffer(b, [N // wmma_n, K // wmma_k, wmma_n, wmma_k])
...
sch.reorder(block_i, block_j, i, j, mma_i_ty, mma_j_tx, ko, ki, ii,
            jj, mma_i, mma_j, kernel_k)
...
sch.compute_at(block_local_A, mma_j, preserve_unit_loops = True)
sch.compute_at(block_local_B, mma_j, preserve_unit_loops = True)
...
def triangle_prmt16x8_32x4(m, n):
    # conficlt 
    # 32 * 128
    start_pos_m = (m // 4) * 4
    start_pos_n = (n // 4) * 4
...
def permutation(i, j, kernel_i, kernel_j):
    return i, j, *triangle_prmt16x8_32x4(kernel_i, kernel_j)

sch.transform_layout(block_shared_A, ("write", 0), permutation)
sch.transform_layout(block_shared_B, ("write", 0), permutation)
...
cuda_b = tvm.nd.array(np.arange(K * N).reshape((N // wmma_n, K // wmma_k, wmma_n, wmma_k)).astype("float32"), ctx)
