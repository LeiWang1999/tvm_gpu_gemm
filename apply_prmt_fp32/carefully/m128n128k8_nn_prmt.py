...
class MyModule:
    ...
    def main(a: T.handle, b: T.handle, c: T.handle):
        ...
        A = T.match_buffer(a, [M // wmma_m, K // wmma_k, wmma_m, wmma_k])
...
mma_i_vy, mma_i_ty, mma_i= sch.split(kernel_i, factors=[2, 8, 1])
mma_j_tx, mma_j = sch.split(kernel_j, factors=[4, 4])
block_i, i, ii = sch.split(i, factors=[None, block_row_warps, warp_row_tiles])
block_j, j, jj = sch.split(j, factors=[None, block_col_warps, warp_col_tiles])
ko, ki = sch.split(k, factors=[None, chunk])
sch.reorder(block_i, block_j, i, j, mma_i_vy, mma_i_ty, mma_j_tx, ko, ki, kernel_k, ii,
            jj, mma_i, mma_j)
mma_warp = sch.fuse(mma_i_ty, mma_j_tx)

sch.bind(block_i, "blockIdx.x")
sch.bind(block_j, "blockIdx.y")
sch.bind(mma_i_vy, "vthread.y")
...
def A_shared_permutation(i, j, kernel_i, kernel_j):
    return i, j, kernel_i // 8, *A_permutation(kernel_i % 8, kernel_j)

sch.transform_layout(block_shared_A,
                     ("write", 0), A_shared_permutation)

# schedule shared A
block_shared_A_fused_i_j = sch.fuse(*sch.get_loops(block_shared_A)[-4:])
A_shared_inner, A_shared_tz, A_shared_ty, A_shared_tx, A_shared_vi = sch.split(
    block_shared_A_fused_i_j, factors=[None, block_col_warps,block_row_warps, warp_size, vec])
...
# schedule local A
block_local_A_fused_i_j = sch.fuse(*sch.get_loops(block_local_A)[-4:])
# _, A_local_vi = sch.split(block_local_A_fused_i_j, factors=[None, vec])
sch.unroll(block_local_A_fused_i_j)
# sch.vectorize(A_local_vi)

# schedule local B
block_local_B_fused_i_j = sch.fuse(*sch.get_loops(block_local_B)[-4:])
_, B_local_vi = sch.split(block_local_B_fused_i_j, factors=[None, vec])
...
cuda_a = tvm.nd.array(np.arange(M * K).reshape((M // wmma_m, K // wmma_k, wmma_m, wmma_k)).astype("float32"), ctx)
