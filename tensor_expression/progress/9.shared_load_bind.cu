  ...
  attr [IterVar(threadIdx.x: int32, (nullptr), "ThreadIndex", "threadIdx.x")] "thread_extent" = 16 {
    ...
    for (k.outer: int32, 0, 1024) {
      for (ax1.outer: int32, 0, 2) {
        ...
        A.shared_1: Buffer(A.shared, float32, [2048], [], scope="shared")[ramp((((threadIdx.y*128) + cse_var_2) + (threadIdx.x*4)), 1, 4)] = A[ramp((((((k.outer*262144) + (threadIdx.y*16384)) + (blockIdx.y*128)) + cse_var_2) + (threadIdx.x*4)), 1, 4)]
      }
      for (ax1.outer_1: int32, 0, 2) {
        let cse_var_3: int32 = (ax1.outer_1*64)
        B.shared_1: Buffer(B.shared, float32, [2048], [], scope="shared")[ramp((((threadIdx.y*128) + cse_var_3) + (threadIdx.x*4)), 1, 4)] = B[ramp((((((k.outer*262144) + (threadIdx.y*16384)) + (blockIdx.x*128)) + cse_var_3) + (threadIdx.x*4)), 1, 4)]
      }
      for (k.inner: int32, 0, 16) {
        for (ax1: int32, 0, 4) {
          A.shared.local_1: Buffer(A.shared.local, float32, [16], [], scope="local", align=16)[ax1] = A.shared_1[(((k.inner*128) + (threadIdx.y*4)) + ax1)]
          A.shared.local_1[(ax1 + 4)] = A.shared_1[((((k.inner*128) + (threadIdx.y*4)) + ax1) + 64)]
        }
        for (ax1_1: int32, 0, 4) {
          B.shared.local_1: Buffer(B.shared.local, float32, [16], [], scope="local", align=16)[ax1_1] = B.shared_1[(((k.inner*128) + (threadIdx.x*4)) + ax1_1)]
