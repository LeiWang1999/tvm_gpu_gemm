@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  attr [IterVar(threadIdx.x: int32, (nullptr), "ThreadIndex", "threadIdx.x")] "thread_extent" = 16 {
    ...
    for (k.outer: int32, 0, 1024) {
      for (ax0: int32, 0, 16) {
        for (ax1: int32, 0, 128) {
          A.shared_1: Buffer(A.shared, float16, [2048], [], scope="shared")[((ax0*128) + ax1)] = A[((((k.outer*262144) + (ax0*16384)) + (blockIdx.y*128)) + ax1)]
        }
      }
      for (ax0_1: int32, 0, 16) {
        for (ax1_1: int32, 0, 128) {
          B.shared_1: Buffer(B.shared, float16, [2048], [], scope="shared")[((ax0_1*128) + ax1_1)] = B[((((k.outer*262144) + (ax0_1*16384)) + (blockIdx.x*128)) + ax1_1)]
        }
      }
      for (k.inner: int32, 0, 16) {
        for (ax1_2: int32, 0, 4) {
          A.shared.local_1: Buffer(A.shared.local, float16, [16], [], scope="local", align=8)[ax1_2] = A.shared_1[(((k.inner*128) + (threadIdx.y*4)) + ax1_2)]
          A.shared.local_1[(ax1_2 + 4)] = A.shared_1[((((k.inner*128) + (threadIdx.y*4)) + ax1_2) + 64)]
        }
        for (ax1_3: int32, 0, 4) {
          B.shared.local_1: Buffer(B.shared.local, float16, [16], [], scope="local", align=8)[ax1_3] = B.shared_1[(((k.inner*128) + (threadIdx.x*4)) + ax1_3)]
