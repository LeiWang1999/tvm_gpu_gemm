...
#endif
extern "C" __global__ void __launch_bounds__(256) default_function_kernel0(half* __restrict__ A, half* __restrict__ B, half* __restrict__ C) {
  half C_local[64];
  __shared__ half A_shared[2048];
  __shared__ half B_shared[2048];
  half A_shared_local[8];
  half B_shared_local[8];
  for (int ii_c_init = 0; ii_c_init < 4; ++ii_c_init) {
    for (int jj_c_init = 0; jj_c_init < 4; ++jj_c_init) {
      C_local[((ii_c_init * 4) + jj_c_init)] = __float2half_rn(0.000000e+00f);
      C_local[(((ii_c_init * 4) + jj_c_init) + 32)] = __float2half_rn(0.000000e+00f);
      C_local[(((ii_c_init * 4) + jj_c_init) + 16)] = __float2half_rn(0.000000e+00f);
      C_local[(((ii_c_init * 4) + jj_c_init) + 48)] = __float2half_rn(0.000000e+00f);
    }
  }
  for (int k_outer = 0; k_outer < 1024; ++k_outer) {
    ...
    for (int ax1_outer = 0; ax1_outer < 2; ++ax1_outer) {
      for (int ax1_inner_inner = 0; ax1_inner_inner < 4; ++ax1_inner_inner) {
        A_shared[((((((int)threadIdx.y) * 128) + (ax1_outer * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner)] = A[((((((k_outer * 262144) + (((int)threadIdx.y) * 16384)) + (((int)blockIdx.y) * 128)) + (ax1_outer * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner)];
      }
    }
    for (int ax1_outer1 = 0; ax1_outer1 < 2; ++ax1_outer1) {
      for (int ax1_inner_inner1 = 0; ax1_inner_inner1 < 4; ++ax1_inner_inner1) {
        B_shared[((((((int)threadIdx.y) * 128) + (ax1_outer1 * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner1)] = B[((((((k_outer * 262144) + (((int)threadIdx.y) * 16384)) + (((int)blockIdx.x) * 128)) + (ax1_outer1 * 64)) + (((int)threadIdx.x) * 4)) + ax1_inner_inner1)];
      }
    }
    __syncthreads();
    for (int k_inner = 0; k_inner < 16; ++k_inner) {
      for (int ax1 = 0; ax1 < 4; ++ax1) {
        A_shared_local[ax1] = A_shared[(((k_inner * 128) + (((int)threadIdx.y) * 4)) + ax1)];
        A_shared_local[(ax1 + 4)] = A_shared[((((k_inner * 128) + (((int)threadIdx.y) * 4)) + ax1) + 64)];
      }
      for (int ax11 = 0; ax11 < 4; ++ax11) {
        B_shared_local[ax11] = B_shared[(((k_inner * 128) + (((int)threadIdx.x) * 4)) + ax11)];
        B_shared_local[(ax11 + 4)] = B_shared[((((k_inner * 128) + (((int)threadIdx.x) * 4)) + ax11) + 64)];
      }
