        ...
        for i, j, k in T.grid(M, N, K):
            with T.block("B"):
                ...
                with T.init():
                    C[vi, vj] = T.float32(0)
                C[vi, vj] = C[vi, vj] + \
                    A[vi, vk].astype("float32") * B[vk, vj].astype("float32")
...
sch.tensorize(block_b_inner_i_tc, MMA_f16f16f32_INTRIN)
...
cuda_c = tvm.nd.array(
    np.zeros((M, N)).astype("float32"), ctx)

if VERIFY:
    cuda_mod(cuda_a, cuda_b, cuda_c)
    c_np = cuda_c.numpy()
    print(c_np)
    print(np.matmul(a_np.astype("float16"), b_np.astype("float16")))
