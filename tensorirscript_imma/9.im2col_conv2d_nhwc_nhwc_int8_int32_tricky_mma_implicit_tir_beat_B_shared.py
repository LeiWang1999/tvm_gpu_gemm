    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        for n, h, w, i in T.grid(batch_size, height + 2*pad_h, width + 2*pad_w, in_channels):
            with T.block("Apad"):
                ...
                Apad[v_n, v_h, v_w, v_i] = T.if_then_else(pad_h <= v_h and v_h < height + pad_h and pad_w <= v_w and v_w < width + pad_w, A[v_n, v_h - pad_h, v_w - pad_w, v_i], T.int8(0), dtype="int8")
        ...
        for xx, yy, kk, x, y, k in T.grid(M // wmma_m, N // wmma_n, K // wmma_k, wmma_m, wmma_n, wmma_k):
            with T.block("Conv"):
                ...
                with T.init():
                    Conv[v_xx, v_yy, v_x, v_y] = T.int8(0)
                Conv[v_xx, v_yy, v_x, v_y] = Conv[v_xx, v_yy, v_x, v_y] + data_im2col[v_xx * wmma_m + v_x, v_kk * wmma_k + v_k].astype("int32") * W[v_yy, v_kk, v_y, v_k].astype("int32")
...
def A_permutation(i, j, kernel_i, kernel_j):
    return (i, j, *shared_load_16x32_to_A_global_16x32_layout(kernel_i, kernel_j))
