...
from intrin.tricky_mma_int8_int32 import (
    ...
    shared_load_16x32_to_A_global_16x32_layout,
...
from intrin.async_copy import ASYNC_COPY_S8_X16_INTRIN
...
class MyModule:
    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        # body
        # with T.block("root")
        A = T.match_buffer(a, (batch_size // wmma_m, height, width, in_channels // wmma_k, wmma_m, wmma_k), dtype="int8")
        W = T.match_buffer(w, (N // wmma_m, K // wmma_k, wmma_m, wmma_k), dtype="int8")
        Conv = T.match_buffer(conv, (M // wmma_m, N // wmma_n, wmma_m, wmma_n), dtype="int32")
        data_im2col = T.alloc_buffer([M // wmma_m, K // wmma_k, wmma_m, wmma_k], dtype="int8")
        ...
        for xx, yy, kk, x, y, k in T.grid(M // wmma_m, N // wmma_n, K // wmma_k, wmma_m, wmma_n, wmma_k):
            with T.block("Conv"):
                ...
                with T.init():
                    Conv[v_xx, v_yy, v_x, v_y] = T.int32(0)
                Conv[v_xx, v_yy, v_x, v_y] = Conv[v_xx, v_yy, v_x, v_y] + data_im2col[v_xx, v_kk, v_x, v_k].astype("int32") * W[v_yy, v_kk, v_y, v_k].astype("int32")
...
def A_permutation(n, h, w, c, kernel_i, kernel_j):
    return (n, h, w, c, *A_global_16x32_to_shared_load_16x32_layout(kernel_i, kernel_j))
...
sch.tensorize(init_block_b_loops[-2], TRICKY_MMA_fill_16x16_i32_INTRIN)
...
tricky_data_shape = (batch_size // wmma_m, height, width, in_channels // wmma_k, wmma_m, wmma_k)
# init a and b to ones
# a_np = np.ones(data_shape).astype("int8")
# b_np = np.ones((N // wmma_m, K // wmma_k, wmma_n, wmma_k)).astype("int8")# random init a and b
...
a_np = np.mod(np.arange(0, batch_size * in_channels * height *
              width), 10).reshape(tricky_data_shape).astype("int8")
b_np = np.mod(np.arange(0, kernel_h * kernel_w * in_channels * out_channels), 10).reshape((N // wmma_m, K // wmma_k, wmma_n, wmma_k)).astype("int8")

c_np = np.zeros((M // wmma_m, N // wmma_n, wmma_m, wmma_n)).astype("int32")
...
if VERIFY:
    ...
    a_np = np.transpose(a_np, (0, 1, 4, 2, 5, 3)).reshape(data_shape)
    ...
    c_torch = torch.nn.functional.conv2d(a_torch, b_torch, groups=1, stride=(stride_h, stride_w))
