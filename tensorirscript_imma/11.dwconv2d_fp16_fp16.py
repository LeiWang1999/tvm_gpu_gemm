...
output_height = (height + 2 * pad_h - dilation_h * (kernel_h - 1) - 1) // stride_h + 1
output_width = (width + 2 * pad_w - dilation_w * (kernel_w - 1) - 1) // stride_w + 1
...
reshape_output = te.compute(
    ...
    lambda c, n, h, w: Conv[te.indexdiv(c, factor), n, h, w, te.indexmod(c, factor)],
    name="reshape_output",
)

ir_module = te.create_prim_func([A, W, reshape_output])
print(ir_module)
