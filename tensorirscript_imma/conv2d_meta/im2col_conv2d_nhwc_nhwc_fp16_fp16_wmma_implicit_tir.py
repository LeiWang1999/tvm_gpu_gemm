...
class MyModule:
    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        for n, h, _w, i in T.grid(batch_size, height + 2*pad_h, width + 2*pad_w, in_channels):
            with T.block("Apad"):
                v_n, v_h, v_w, v_i = T.axis.remap("SSSS", [n, h, _w, i])
...
b3 = sch.reindex(block=block_conv, buffer=("write", 0))
b4 = sch.reindex(block=block_conv, buffer=("read", 0))
b5 = sch.reindex(block=block_conv, buffer=("read", 1))
...
sch.compute_inline(b4)
sch.compute_inline(b5)
...
A_shared_j, A_shared_vi = sch.split(sch.get_loops(
    block_conv_input_shared)[-1], factors=[None, vec])
ba = sch.blockize(A_shared_vi)
...
sch.storage_align(block_conv_input_shared, 0, axis=-2, factor=32, offset=vec)
...
sch.storage_align(block_conv_weight_shared, 0, axis=-2, factor=32, offset=vec)
...
a_np = (np.mod(np.arange(0, batch_size * in_channels * height * width), 23) / 100).reshape(data_shape).astype("float16")
b_np = (np.mod(np.arange(0, kernel_h * kernel_w * in_channels * out_channels), 12) / 100).reshape(kernel_shape).astype("float16")
