...
write_sch(sch, log_path,
          "tensorize_fill")
sch.tensorize(sch.get_loops(block_conv_input_frag)[-2], WMMA_LOAD_16x16x16_S8_A_INTRIN)
write_sch(sch, log_path,
          "tensorize_load")
sch.tensorize(sch.get_loops(block_conv_weight_frag)[-2], WMMA_LOAD_16x16x16_S8_B_TRANS_INTRIN)
...
b_np = np.mod(np.arange(0, kernel_h * kernel_w * in_channels * out_channels),
              10).reshape((N // wmma_m, K // wmma_k, wmma_n, wmma_k)).astype("int8")
