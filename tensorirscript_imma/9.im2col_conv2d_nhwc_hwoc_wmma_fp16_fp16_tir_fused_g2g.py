...
class MyModule:
    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        W = T.match_buffer(w, (kernel_h, kernel_w, in_channels, out_channels), dtype="float16")
        ...
        data_im2colPad = T.alloc_buffer([MPAD, KPAD], dtype="float16")
        ...
        for x, y in T.grid(K, N):
            with T.block("weight_flatten"):
                v_x, v_y = T.axis.remap("SS", [x, y])
                weight_flatten[v_x, v_y] = W[(v_x // in_channels) // kernel_w, (v_x // in_channels) % kernel_w, v_x % in_channels, v_y]
        
        for i, k in T.grid(MPAD, KPAD):
            with T.block("data_im2colPad"):
                vi, vk = T.axis.remap("SS", [i, k])
                data_im2colPad[vi, vk] = T.if_then_else(vi < M and vk < K, data_im2col[vi, vk], T.float16(0), dtype="float16")
        ...
        for x, y, k in T.grid(MPAD, NPAD, KPAD):
            with T.block("Conv"):
                ...
                with T.init():
                    CPad[v_x, v_y] = T.float16(0)
                CPad[v_x, v_y] = CPad[v_x, v_y] + data_im2colPad[v_x, v_k] * weight_flattenPad[v_k, v_y]

        
        for i, j in T.grid(M, N):
            with T.block("CPad"):
                vi, vj = T.axis.remap("SS", [i, j])
                Conv[vi, vj] = CPad[vi, vj]
...
sch.reverse_compute_at(block_conv_output_shared, j, preserve_unit_loops=True)
...
# sch.reverse_compute_at(block_conv_output_shared, sch.get_loops(block_conv_output_frag)[-3], preserve_unit_loops=True)
