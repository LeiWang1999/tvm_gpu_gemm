...
in_channels_pad = KPAD // (kernel_h * kernel_w)
...
class MyModule:
    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        Conv = T.match_buffer(conv, (MPAD, NPAD), dtype="float16")
        data_im2col = T.alloc_buffer([MPAD, KPAD], dtype="float16")
        weight_flatten = T.alloc_buffer([NPAD, KPAD], dtype="float16")

        
        for x, y in T.grid(M, K):
            with T.block("data_im2col"):
                v_x, v_y = T.axis.remap("SS", [x, y])
                T.writes(data_im2col[0:MPAD, 0:KPAD])
                data_im2col[v_x, v_y] = T.if_then_else(0 <= stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels_pad // 1) and stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels_pad // kernel_w) < output_width and 0 <= stride_w * (v_x % in_channels_pad % output_width) + dilation_w * (v_y // in_channels_pad % kernel_w) and stride_w * (v_x % (output_height * output_width) % output_width) + dilation_w * (v_y // in_channels_pad % kernel_w) < output_width, A[v_x // (output_height * output_width), stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels_pad // kernel_w) - pad_h, stride_w * (v_x % (output_height * output_width) % output_width) + dilation_w * (v_y // in_channels_pad % kernel_w) - pad_w, v_y % in_channels_pad], T.float16(0), dtype="float16")

        
        for x, y in T.grid(N, K):
            with T.block("weight_flatten"):
                v_n, v_k = T.axis.remap("SS", [x, y])
                T.writes(weight_flatten[0:NPAD, 0:KPAD])
                weight_flatten[v_n, v_k] = W[v_n, (v_k // in_channels) // kernel_w, (v_k // in_channels) % kernel_w, v_k % in_channels]
        

        for x, y, k in T.grid(MPAD, NPAD, KPAD):
            with T.block("Conv"):
...
def B_permutation(n, h, w, c):
    ...
    kernel_i, kernel_j = B_global_16x16_to_shared_load_16x16_layout(
        i % wmma_n, j % wmma_k)
...
write_sch(sch, log_path, "transform_g2s_layout")
...
c_np = np.zeros((MPAD, NPAD)).astype("float16")
...
if VERIFY:
    ...
    tvm_c_np = cuda_c.asnumpy()
    # crop tvm_c_np from [MPAD, NPAD] to [M, N]
    tvm_c_np = tvm_c_np[:M, :N]
    print("torch result: ", c_torch_np[0][0:10])
    print("tvm result: ", tvm_c_np[0][0:10])
    np.testing.assert_allclose(c_torch_np, tvm_c_np, rtol=1e-1)
