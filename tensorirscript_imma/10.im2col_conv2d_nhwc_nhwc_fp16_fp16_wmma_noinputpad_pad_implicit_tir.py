...
class MyModule:
    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        data_im2col = T.alloc_buffer([MPAD, KPAD], dtype="float16", scope="shared")
        weight_flatten = T.alloc_buffer([NPAD, KPAD], dtype="float16", scope="shared")
        
        for x, y in T.grid(M, K):
            with T.block("data_im2col"):
                v_x, v_y = T.axis.remap("SS", [x, y])
                T.writes(data_im2col[0:MPAD, 0:KPAD])
                data_im2col[v_x, v_y] = A[v_x // (output_height * output_width), stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels // kernel_w) - pad_h, stride_w * (v_x % (output_height * output_width) % output_width) + dilation_w * (v_y // in_channels % kernel_w) - pad_w, v_y % in_channels]
...
block_conv_input_shared = sch.get_block("data_im2col")
block_conv_weight_shared = sch.get_block("weight_flatten")
