...
Conv = te.compute(
    (MPAD, NPAD),
    lambda x, y: te.sum(A_imp_pad[x, k] * W_flat_pad[k, y], axis=k),
    name="Conv",
)
...
block_im2col_pad = sch.get_block("A_imp_pad")
block_flat = sch.get_block("weight_flatten")
block_flat_pad = sch.get_block("W_flat_pad")
...
if VERIFY:
    ...
    c_torch_np = c_torch.cpu().numpy().reshape((M, N))
