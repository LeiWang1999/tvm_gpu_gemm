    ...
    def main(a: T.handle, w: T.handle, conv: T.handle):
        ...
        for x, y in T.grid(M, K):
            with T.block("data_im2col"):
                ...
                data_im2col[v_x, v_y] = T.if_then_else(0 <= stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels // 1) and stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels // kernel_w) < output_width and 0 <= stride_w * (v_x % in_channels % output_width) + dilation_w * (v_y // in_channels % kernel_w) and stride_w * (v_x % (output_height * output_width) % output_width) + dilation_w * (v_y // in_channels % kernel_w) < output_width, A[v_x // (output_height * output_width), stride_h * (v_x % (output_height * output_width) // output_width) + dilation_h * (v_y // in_channels // kernel_w) - pad_h, stride_w * (v_x % (output_height * output_width) % output_width) + dilation_w * (v_y // in_channels % kernel_w) - pad_w, v_y % in_channels], T.float16(0), dtype="float16")
