...
def test_gemm():
    ...
    # bind buffer
    A_shared_buffer = tvm.tir.decl_buffer(A.shape, A.dtype, "shared", data_alignment=16)
    B_shared_buffer = tvm.tir.decl_buffer(B.shape, B.dtype, "shared", data_alignment=16)
    C_local_buffer = tvm.tir.decl_buffer(C.shape, C.dtype, "local", data_alignment=16)
    write_code(
        str(tvm.lower(s, [A, B, C], simple_mode=True, binds={AA: A_shared_buffer, BB: B_shared_buffer, CC: C_local_buffer})), "progress/10.bind buffer.cu")
    ...
    write_code(f.imported_modules[0].get_source(), "progress/11.tmp.cu")
    # launch the kernel.
    n, m, l = nn, nn, nn
