...
def sum(M, K, in_dtype, out_dtype):
    A = tvm.te.placeholder([M, K], dtype=in_dtype, name="A")
    
    rk = tvm.te.reduce_axis([0, K], name="k")
    if out_dtype == "float16":
        B = tvm.te.compute(
            [M], lambda i: tvm.te.sum((A[i, rk]).astype(out_dtype), axis=rk), name="B"
        )
        
    return [A, B]


A, Sum = sum(M, K, in_dtype, out_dtype)
...
if VERIFY:
    import torch
    a_torch = torch.tensor(a_np, dtype=torch.float16)
    b_torch = torch.sum(a_torch, dim=[1], dtype=torch.float16)
