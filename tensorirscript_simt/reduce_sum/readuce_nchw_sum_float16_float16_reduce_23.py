...
# sch.reorder(bx, i, tz, ty, k, tx, kernel_k)
...
# a_np = np.mod(np.arange(M * K), 4).reshape((batch_size, in_channels, height, width)).astype("float16")
b_np = np.zeros((batch_size * in_channels)).astype("float16")

cuda_a = tvm.nd.array((a_np).astype("float16"), ctx)
cuda_c = tvm.nd.array((b_np).astype("float16"), ctx)
...
if VERIFY:
    ...
    np.testing.assert_allclose(
        cuda_c.asnumpy(), b_torch.numpy().reshape([-1]), rtol=1e-3)
