...
def mean(M, K, in_dtype, out_dtype):
    ...
    T = tvm.te.compute([M], lambda i: te.const(
            1.0 / K, dtype=cast_dtype), name="T")
    rk = tvm.te.reduce_axis([0, K], name="k")
    B = tvm.te.compute(
        [M], lambda i: tvm.te.sum((A[i, rk]).astype(cast_dtype) * T[i], axis=rk), name="B"
    )
    C = tvm.te.compute([M], lambda i: B[i].astype(out_dtype), name="C")
    return [A, B, C]


A, _, Mean = mean(M, K, in_dtype, out_dtype)
...
a_np = np.mod(np.arange(M * K), 4).reshape((M, K)).astype("int8")
