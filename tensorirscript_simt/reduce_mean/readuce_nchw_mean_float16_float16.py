...
def mean(N, C, H, W, in_dtype, out_dtype):
    ...
    T = tvm.te.compute([C], lambda i: te.const(
        1.0 / (N * H * W), dtype=in_dtype), name="T")
    rk = tvm.te.reduce_axis([0, N * H * W], name="k")
    _C = tvm.te.compute(
        [C], lambda i: tvm.te.sum((B[i, rk]).astype(out_dtype) * T[i], axis=rk), name="C"
    )
    return [A, B, _C]
...
a_np = np.mod(np.arange(M * K), 4).reshape((batch_size,
                                            in_channels, height, width)).astype("float16")
...
if VERIFY:
    ...
    np.testing.assert_allclose(cuda_c.asnumpy(), b_torch.numpy(), rtol=1e-1)
