...
import os
from intrin.tricky_mma_int8_int32_quantize import (
...
log_path = "progress/amos_with_tensorir_quantize_4terms/4.tricky_mma_int8_int32_nt"
count = 0
...
class MyModule:
    ...
    def main(a: T.handle, b: T.handle, c: T.handle):
        ...
        for j, k in T.grid(N, K):
            with T.block("Pre_compute_B"):
                vj, vk = T.axis.remap("SR", [j, k])
                with T.init():
                    PB[vj] = 0
                PB[vj] = PB[vj] + Z1 * B[vj, vk].astype("int32")

        for i, j in T.grid(M, N):
            with T.block("C"):
                vi, vj = T.axis.remap("SS", [i, j])
                C[vi, vj] = QC[vi, vj] - PA[vi] - PB[vj] + Z0*Z1
...
block_pre_compute_b = sch.get_block("Pre_compute_B")
