                ...
                for j_0_1 in T.thread_binding(2, thread="threadIdx.z"):
                        ...
                        for k_0_1 in T.serial(2):
                            ...
                            for ax0_0, ax1_0, ax0_1, ax1_1 in T.grid(8, 1, 16, 16):
                                with T.block("B_global_shared_wmma.matrix_b"):
                                    v0 = T.axis.spatial(16384, j_0_0 * 256 + j_0_1 * 128 + ax0_0 * 16 + ax0_1)
                            ...
                            for i_0_2, j_0_2, i_1, j_1, k_1 in T.grid(2, 8, 16, 16, 16):
                                with T.block("B_update"):
                                    ...
                                    vj = T.axis.spatial(16384, j_0_0 * 256 + j_0_1 * 128 + j_0_2 * 16 + j_1)
                                    vk = T.axis.reduce(16384, k_0_0 * 32 + k_0_1 * 16 + k_1)
                                    T.reads(C_global_wmma_accumulator[vi // 16, vj // 16, vi % 16, vj % 16], A_global_shared_wmma_matrix_a[vi // 16, vk // 16, vi % 16, vk % 16], B_global_shared_wmma_matrix_b[vj // 16, vk // 16, vj % 16, vk % 16])
                                    T.writes(C_global_wmma_accumulator[vi // 16, vj // 16, vi % 16, vj % 16])
                                    C_global_wmma_accumulator[vi // 16, vj // 16, vi % 16, vj % 16] = C_global_wmma_accumulator[vi // 16, vj // 16, vi % 16, vj % 16] + T.Cast("int32", A_global_shared_wmma_matrix_a[vi // 16, vk // 16, vi % 16, vk % 16]) * T.Cast("int32", B_global_shared_wmma_matrix_b[vj // 16, vk // 16, vj % 16, vk % 16])
                    for ax0_0, ax1_0, ax0_1, ax1_1 in T.grid(2, 8, 16, 16):
                        with T.block("C_global_wmma.accumulator"):
                            ...
                            v1 = T.axis.spatial(16384, j_0_0 * 256 + j_0_1 * 128 + ax1_0 * 16 + ax1_1)
                            T.reads(C_global_wmma_accumulator[v0 // 16, v1 // 16, v0 % 16, v1 % 16])
                            T.writes(C_global[v0 // 16, v1 // 16, v0 % 16, v1 % 16])
                            C_global[v0 // 16, v1 // 16, v0 % 16, v1 % 16] = C_global_wmma_accumulator[v0 // 16, v1 // 16, v0 % 16, v1 % 16]
    for ax0, ax1 in T.grid(16384, 16384):
