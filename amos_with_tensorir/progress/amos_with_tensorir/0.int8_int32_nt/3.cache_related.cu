...
@main = primfn(a: handle, b: handle, c: handle) -> ()
  ...
  block([], "root") {
    ...
    C_global_wmma.accumulator = alloc_buffer(int32[16384, 16384])
     {
        ...
        for (j: int32, 0, 16384) {
          for (k: int32, 0, 16384) {
            block([16384, 16384, tir.reduce_axis(0, 16384)], "B") as [vi, vj, vk] {
              ...
              tir.writes([C_global_wmma.accumulator[vi, vj]])
              with init() {
                C_global_wmma.accumulator[vi, vj] = 0
              }
              C_global_wmma.accumulator[vi, vj] = (C_global_wmma.accumulator[vi, vj] + (cast(int32, A_global_shared_wmma.matrix_a[vi, vk])*cast(int32, B_global_shared_wmma.matrix_b[vj, vk])))
          }
        }
      }
      for (ax0_6: int32, 0, 16384) {
        for (ax1_6: int32, 0, 16384) {
          block([16384, 16384], "C_global_wmma.accumulator") as [v0_6, v1_6] {
            bind(v0_6, ax0_6)
            bind(v1_6, ax1_6)
            tir.reads([C_global_wmma.accumulator[v0_6, v1_6]])
            tir.writes([C_global[v0_6, v1_6]])
            C_global[v0_6, v1_6] = C_global_wmma.accumulator[v0_6, v1_6]
        }
      }
      for (ax0_7: int32, 0, 16384) {
        for (ax1_7: int32, 0, 16384) {
          block([16384, 16384], "C_global") as [v0_7, v1_7] {
            bind(v0_7, ax0_7)
            bind(v1_7, ax1_7)
            tir.reads([C_global[v0_7, v1_7]])
            tir.writes([C[v0_7, v1_7]])
            C[v0_7, v1_7] = C_global[v0_7, v1_7]
        }
      }
    }
}
