@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(C.wmma.accumulator: Pointer(wmma.accumulator float32), float32, [4096]), storage_scope = wmma.accumulator {
    ...
    for (ii.c: int32, 0, 64) {
      for (jj.c: int32, 0, 64) {
        C.wmma.accumulator_1: Buffer(C.wmma.accumulator, float32, [4096], [], scope="wmma.accumulator")[((ii.c*64) + jj.c)] = 0f32
        for (rk: int32, 0, 32) {
          let cse_var_5: int32 = ((ii.c*64) + jj.c)
      ...
      for (jj.inner.outer.inner: int32, 0, 4) {
        for (ii.inner.inner: int32, 0, 16) {
          for (jj.inner.inner: int32, 0, 16) {
            let cse_var_6: int32 = (jj.inner.outer.inner*16)
            C[((((((blockIdx.y*16384) + (ii.inner.outer.inner*4096)) + (ii.inner.inner*256)) + (blockIdx.x*64)) + cse_var_6) + jj.inner.inner)] = C.wmma.accumulator_1[((((ii.inner.outer.inner*1024) + (ii.inner.inner*64)) + cse_var_6) + jj.inner.inner)]
          }
        }
      }
    }
