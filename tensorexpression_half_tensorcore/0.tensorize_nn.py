...
def test_gemm():
    ...
    write_code(
        str(tvm.lower(s, [A, B, C], simple_mode=True)), "progress/4.split_thread.cu")
    ...
    write_code(
        str(tvm.lower(s, [A, B, C], simple_mode=True)), "progress/4.compute_at_cf.cu")
    ...
    write_code(
        str(tvm.lower(s, [A, B, C], simple_mode=True)), "progress/4.compute_at_af.cu")
    ...
    write_code(
        str(tvm.lower(s, [A, B, C], simple_mode=True)), "progress/4.compute_at_bf.cu")
    ...
    def shared_schedule(stage, strides):
        ...
        fused_xo_yo = s[stage].fuse(xo, yo)
        # t, vi = s[stage].split(t, factor=vec)
        fused_xo_yo, tz = s[stage].split(fused_xo_yo, factor=block_row_warps)
        fused_xo_yo, ty = s[stage].split(fused_xo_yo, factor=block_col_warps)
        # s[stage].reorder(tz, ty, xo, yo)
        # fused_xo_yo = s[stage].fuse(xo, yo)
        fused_xo_yo, tx= s[stage].split(fused_xo_yo, factor=warp_size)
