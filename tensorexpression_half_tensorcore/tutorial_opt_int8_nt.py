...
import os
log_path = "progress/tensorexpression_half_tensorcore/tutorial_opt"
count = 0
...
def write_sch(ir_module, path, fname):
...
Conv = te.compute(
    ...
    lambda n, h, w, o, nn, oo: te.sum(
        A[n, h * stride_h + kh, w * stride_w + kw, ic, nn, ii].astype("int32")
        * W[o, kh, kw, ic, oo, ii].astype("int32"),
        axis=[ic, kh, kw, ii],
    ),
    name="Conv",
)
...
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "lower.cu")
...
AS = s.cache_read(A, "shared", [Conv])
WS = s.cache_read(W, "shared", [Conv])
AF = s.cache_read(AS, "wmma.matrix_a", [Conv])
WF = s.cache_read(WS, "wmma.matrix_b", [Conv])
ConvF = s.cache_write(Conv, "wmma.accumulator")
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "cache_related.cu")
###############################################################################
...
def intrin_wmma_load_matrix(scope):
    n = 16
    A = te.placeholder((n, n), name="A", dtype="int8")
...
def intrin_wmma_gemm():
    n = 16
    A = te.placeholder((n, n), name="A", dtype="int8")
    B = te.placeholder((n, n), name="B", dtype="int8")
    k = te.reduce_axis((0, n), name="k")
    C = te.compute(
        (n, n),
        lambda ii, jj: te.sum(A[ii, k].astype("int32") * B[jj, k].astype("int32"), axis=k),
        name="C",
    )
...
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "bind_block.cu")

s[Conv].reorder(block_i, block_j, nc, oc, nci, oci, nnc, ooc)
...
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "schedule_output_frag.cu")

# Move intermediate computation into each output compute tile
s[AF].compute_at(s[ConvF], kw)
s[WF].compute_at(s[ConvF], kw)
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "compute_at_input_frag.cu")

# Schedule for A's share memory
s[AS].compute_at(s[ConvF], kh)
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "schedule_A_shared.cu")
n, h, w, i, nn, ii = AS.op.axis
tx, xo = s[AS].split(n, nparts=block_row_warps)
ty, yo = s[AS].split(xo, nparts=block_col_warps)
t = s[AS].fuse(i, nn, ii)
# to, ti = s[AS].split(t, nparts=warp_size)
ti, tv = s[AS].split(t, factor=16)
_, to = s[AS].split(ti, factor=warp_size)
...
o, kh, kw, ic, oo, ii = WS.op.axis
o = s[WS].fuse(o, kh, kw, ic, oo, ii)
tx, xo = s[WS].split(o, nparts=block_row_warps)
ty, yo = s[WS].split(xo, nparts=block_col_warps)
ti, tv = s[WS].split(yo, factor=16)
_, to = s[WS].split(ti, factor=warp_size)
s[WS].bind(tx, thread_y)
s[WS].bind(ty, thread_z)
s[WS].bind(to, thread_x)
s[WS].vectorize(tv)
write_sch(tvm.lower(s, [A, W, Conv], simple_mode=True), log_path, "schedule_B_shared.cu")
...
if nvcc.have_tensorcore(dev.compute_version):
    ...
    write_code(func.imported_modules[0].get_source(), log_path, "tensorcore.cu")
