...
def func(A: T.Buffer[(16384, 16384), "int8"], B: T.Buffer[(16384, 16384), "int8"], C: T.Buffer[(16384, 16384), "int8"]):
    ...
    QA_shared = T.alloc_buffer([16384, 16384], dtype="int8", scope="shared")
    QA_shared_wmma_matrix_a = T.alloc_buffer([16384, 16384], dtype="int8", scope="wmma.matrix_a")
    B_global = T.alloc_buffer([16384, 16384], dtype="int8")
    QB_shared = T.alloc_buffer([16384, 16384], dtype="int8", scope="shared")
    QB_shared_wmma_matrix_b = T.alloc_buffer([16384, 16384], dtype="int8", scope="wmma.matrix_b")
    ...
    for ax0, ax1 in T.grid(16384, 16384):
        with T.block("QA_shared"):
            v0, v1 = T.axis.remap("SS", [ax0, ax1])
            T.reads(QA[v0, v1])
            T.writes(QA_shared[v0, v1])
            QA_shared[v0, v1] = QA[v0, v1]
    for ax0, ax1 in T.grid(16384, 16384):
        with T.block("QA_shared_wmma.matrix_a"):
            v0, v1 = T.axis.remap("SS", [ax0, ax1])
            T.reads(QA_shared[v0, v1])
            T.writes(QA_shared_wmma_matrix_a[v0, v1])
            QA_shared_wmma_matrix_a[v0, v1] = QA_shared[v0, v1]
    for ax0, ax1 in T.grid(16384, 16384):
        with T.block("QB_shared"):
            v0, v1 = T.axis.remap("SS", [ax0, ax1])
            T.reads(QB[v0, v1])
            T.writes(QB_shared[v0, v1])
            QB_shared[v0, v1] = QB[v0, v1]
    for ax0, ax1 in T.grid(16384, 16384):
        with T.block("QB_shared_wmma.matrix_b"):
            v0, v1 = T.axis.remap("SS", [ax0, ax1])
            T.reads(QB_shared[v0, v1])
            T.writes(QB_shared_wmma_matrix_b[v0, v1])
            QB_shared_wmma_matrix_b[v0, v1] = QB_shared[v0, v1]
    for i, j, k in T.grid(16384, 16384, 16384):
        with T.block("B"):
            vi, vj, vk = T.axis.remap("SSR", [i, j, k])
            T.reads(QA_shared_wmma_matrix_a[vi, vk], QB_shared_wmma_matrix_b[vj, vk])
            T.writes(QC_wmma_accumulator[vi, vj])
            with T.init():
                QC_wmma_accumulator[vi, vj] = 0
            QC_wmma_accumulator[vi, vj] = QC_wmma_accumulator[vi, vj] + T.Cast("int32", QA_shared_wmma_matrix_a[vi, vk]) * T.Cast("int32", QB_shared_wmma_matrix_b[vj, vk])
    for ax0, ax1 in T.grid(16384, 16384):
