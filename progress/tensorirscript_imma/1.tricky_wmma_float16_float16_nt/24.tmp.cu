...
extern "C" __global__ void __launch_bounds__(256) main_kernel0(half* __restrict__ A, half* __restrict__ B, half* __restrict__ C) {
  ...
  for (int kk_0 = 0; kk_0 < 512; ++kk_0) {
    __syncthreads();
    *(uint4*)(A_shared + (((((int)threadIdx.y) * 1024) + (((int)threadIdx.z) * 256)) + (((int)threadIdx.x) * 8))) = *(uint4*)(A + ((((((((int)blockIdx.y) * 1048576) + (((int)threadIdx.y) * 524288)) + ((((int)threadIdx.z) >> 1) * 262144)) + (kk_0 * 512)) + ((((int)threadIdx.z) & 1) * 256)) + (((int)threadIdx.x) * 8)));
    for (int ax0_ax1_ax2_ax3_fused_2 = 0; ax0_ax1_ax2_ax3_fused_2 < 4; ++ax0_ax1_ax2_ax3_fused_2) {
      *(uint4*)(B_shared + ((((((int)threadIdx.y) * 4096) + (((int)threadIdx.z) * 1024)) + (ax0_ax1_ax2_ax3_fused_2 * 256)) + (((int)threadIdx.x) * 8))) = *(uint4*)(B + (((((((((int)blockIdx.x) * 4194304) + (((int)threadIdx.y) * 2097152)) + (((int)threadIdx.z) * 524288)) + ((ax0_ax1_ax2_ax3_fused_2 >> 1) * 262144)) + (kk_0 * 512)) + ((ax0_ax1_ax2_ax3_fused_2 & 1) * 256)) + (((int)threadIdx.x) * 8)));
    }
    __syncthreads();
    for (int kk_1 = 0; kk_1 < 2; ++kk_1) {
      ...
      for (int ax0_1 = 0; ax0_1 < 4; ++ax0_1) {
        nvcuda::wmma::load_matrix_sync(B_shared_wmma_matrix_b[ax0_1], (&(B_shared[(((((int)threadIdx.z) * 2048) + (ax0_1 * 512)) + (kk_1 * 256))])), 16);
      }
  ...
  for (int ax0_2 = 0; ax0_2 < 2; ++ax0_2) {
    for (int ax1 = 0; ax1 < 4; ++ax1) {
      nvcuda::wmma::store_matrix_sync((&(C[((((((((int)blockIdx.y) * 1048576) + (((int)threadIdx.y) * 524288)) + (ax0_2 * 262144)) + (((int)blockIdx.x) * 4096)) + (((int)threadIdx.z) * 1024)) + (ax1 * 256))])), C_wmma_accumulator[((ax0_2 * 4) + ax1)], 16, nvcuda::wmma::mem_row_major);
    }
  }
}
