...
;
  ...
  for (int k_0 = 0; k_0 < 4; ++k_0) {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0) {
      *(int4*)(A_shared + (((ax0_ax1_fused_0 * 2048) + (((int)threadIdx.y) * 512)) + (((int)threadIdx.x) * 16))) = *(int4*)(A + ((((((((int)blockIdx.y) * 32768) + (ax0_ax1_fused_0 * 8192)) + (((int)threadIdx.y) * 2048)) + ((((int)threadIdx.x) >> 2) * 256)) + (k_0 * 64)) + ((((int)threadIdx.x) & 3) * 16)));
    }
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 8; ++ax0_ax1_fused_0_1) {
      *(int4*)(B_shared + (((ax0_ax1_fused_0_1 * 2048) + (((int)threadIdx.y) * 512)) + (((int)threadIdx.x) * 16))) = *(int4*)(B + ((((k_0 * 16384) + (ax0_ax1_fused_0_1 * 2048)) + (((int)threadIdx.y) * 512)) + (((int)threadIdx.x) * 16)));
    }
    __syncthreads();
...

  {
    ...
    __asm__ __volatile__(
      ...
      : "r"(addr)
          ...
          for (int i = 0; i < 16; ++i) {
B_shared_warp[0 + i] = (&(B_shared[(((k_1_0 * 8192) + (((int)threadIdx.y) * 64)) + (j_1_1_0 * 16))]))[(i % 8) / 4 * 256 * 16 + (threadIdx.x % 4) * 4 * 256+ (i % 4) * 256 + threadIdx.x / 4 +  (i / 8) * 8];
}
;
...
(&(C[(((((((int)blockIdx.y) * 32768) + (((int)threadIdx.z) * 16384)) + (ax0_0 * 4096)) + (((int)threadIdx.y) * 64)) + (ax1_0 * 16))]))[((((((local_id % 4) / 2) * 8) + (threadIdx.x / 4)) * 256) + ((((local_id / 4) * 8) + ((threadIdx.x % 4) * 2)) + (local_id % 2)))] = C_warp[((ax0_0 * 32) + (ax1_0 * 8)) + local_id];
}
;
    }
  }
