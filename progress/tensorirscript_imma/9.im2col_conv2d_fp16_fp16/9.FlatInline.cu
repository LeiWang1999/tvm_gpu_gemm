      ...
      for (k_1: int32, 0, 640) {
        for (j: int32, 0, 128) {
          block([640, 128], "weight_flattenPad") as [vk_1, vj] {
            bind(vk_1, k_1)
            bind(vj, j)
            tir.reads([W[floordiv(vk_1, 192), floordiv(floormod(vk_1, 192), 64), floormod(vk_1, 64), vj]])
            tir.writes([weight_flattenPad[vk_1, vj]])
            weight_flattenPad[vk_1, vj] = @tir.if_then_else(((vk_1 < 576) && (vj < 64)), W[floordiv(floordiv(vk_1, 64), 3), floormod(floordiv(vk_1, 64), 3), floormod(vk_1, 64), vj], 0f16, dtype=float16)
        }
      }
      for (ax0: int32, 0, 1) {
