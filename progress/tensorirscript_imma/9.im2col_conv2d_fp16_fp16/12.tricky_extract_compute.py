...
def func(A: T.Buffer[(1, 224, 224, 256), "float16"], W: T.Buffer[(7, 7, 256, 512), "float16"], Conv: T.Buffer[(1, 48400, 512), "float16"]):
    ...
    for n, x_0, y_0, k_0, x_1, y_1, k_1 in T.grid(1, 3025, 32, 784, 16, 16, 16):
        with T.block("Conv"):
            v_n = T.axis.spatial(1, n)
            v_x = T.axis.spatial(48400, x_0 * 16 + x_1)
            v_y = T.axis.spatial(512, y_0 * 16 + y_1)
            v_k = T.axis.reduce(12544, k_0 * 16 + k_1)
