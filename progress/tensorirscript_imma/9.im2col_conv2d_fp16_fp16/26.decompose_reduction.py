                ...
                for y_0_0_1 in T.thread_binding(16, thread="blockIdx.x"):
                    for x_0_1 in T.thread_binding(1, thread="threadIdx.y"):
                        for y_0_1 in T.thread_binding(1, thread="threadIdx.z"):
                            for x_0_2_init, y_0_2_init, x_1_init, y_1_init in T.grid(1, 1, 16, 16):
                                with T.block("Conv_init"):
                                    v_n = T.axis.spatial(1, n)
                                    v_x = T.axis.spatial(48400, x_0_0 * 16 + x_0_1 * 16 + x_0_2_init * 16 + x_1_init)
                                    v_y = T.axis.spatial(512, y_0_0_0 * 256 + y_0_0_1 * 16 + y_0_1 * 16 + y_0_2_init * 16 + y_1_init)
                                    T.reads()
                                ...
                                for k_0_1 in T.serial(4):
                                    for ax0_0, ax1_0, ax0_1, ax1_1 in T.grid(1, 1, 16, 16):
                                        with T.block("data_im2col_global_shared_wmma.matrix_a"):
                                            ...
                                            v2 = T.axis.spatial(12544, k_0_0 * 64 + k_0_1 * 16 + ax1_0 * 16 + ax1_1)
                                    ...
                                    for x_0_2, y_0_2, x_1, y_1, k_1 in T.grid(1, 1, 16, 16, 16):
                                        with T.block("Conv_update"):
                                            ...
                                            T.reads(Conv_wmma_accumulator[v_n, v_x // 16, v_y // 16, v_x % 16, v_y % 16], data_im2col_global_shared_wmma_matrix_a[v_n, v_x // 16, v_k // 16, v_x % 16, v_k % 16], weight_flatten_global_shared_wmma_matrix_b[v_k // 16, v_y // 16, v_k % 16, v_y % 16])
                                            T.writes(Conv_wmma_accumulator[v_n, v_x // 16, v_y // 16, v_x % 16, v_y % 16])
