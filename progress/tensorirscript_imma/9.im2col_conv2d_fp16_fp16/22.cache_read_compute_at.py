                    ...
                    for y_0_1 in T.thread_binding(4, thread="threadIdx.z"):
                        for k_0_0 in T.serial(20, annotations={"thread_rasterization":1}):
                            for ax0, ax1 in T.grid(128, 32):
                                with T.block("data_im2colPad_shared"):
                                    ...
                                    v1 = T.axis.spatial(3200, x_0_0 * 128 + ax0)
                                    v2 = T.axis.spatial(640, k_0_0 * 32 + ax1)
                            ...
                            for ax0, ax1 in T.grid(32, 128):
                                with T.block("weight_flattenPad_shared"):
                                    v0 = T.axis.spatial(640, k_0_0 * 32 + ax0)
                            ...
                            for k_0_1 in T.serial(2):
                                for ax0, ax1 in T.grid(128, 16):
                                    with T.block("data_im2colPad_shared_wmma.matrix_a"):
                                        v0 = T.axis.spatial(1, 0)
                                        v1 = T.axis.spatial(3200, x_0_0 * 128 + ax0)
                                        v2 = T.axis.spatial(640, k_0_0 * 32 + k_0_1 * 16 + ax1)
                                ...
                                for ax0, ax1 in T.grid(16, 32):
                                    with T.block("weight_flattenPad_shared_wmma.matrix_b"):
                                        v0 = T.axis.spatial(640, k_0_0 * 32 + k_0_1 * 16 + ax0)
                                        v1 = T.axis.spatial(128, y_0_1 * 32 + ax1)
                        ...
                        for ax0, ax1 in T.grid(128, 32):
                            with T.block("CPad_shared_wmma.accumulator"):
                                v0 = T.axis.spatial(1, 0)
                                v1 = T.axis.spatial(3200, x_0_0 * 128 + ax0)
                                v2 = T.axis.spatial(128, y_0_1 * 32 + ax1)
