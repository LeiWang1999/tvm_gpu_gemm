                        ...
                        for y_0_1 in T.thread_binding(1, thread="threadIdx.z"):
                            for x_0_2_init, y_0_2_init in T.grid(1, 1):
                                with T.block("Conv_init_o"):
                                    ...
                                    T.reads()
                                    T.writes(Conv_wmma_accumulator[v_n, v_x_o, v_y_o, 0 : 16, 0 : 16])
                                    C = T.match_buffer(Conv_wmma_accumulator[v_n, v_x_o, v_y_o, 0 : 16, 0 : 16], [16, 16], dtype="float16", strides=[C_s0, C_s1], scope="wmma.accumulator", offset_factor=16)
                                ...
                                for k_0_1 in T.serial(4):
                                    ...
                                    for x_0_2, y_0_2 in T.grid(1, 1):
                                        with T.block("Conv_update_o"):
                                            ...
                                            T.reads(Conv_wmma_accumulator[v_n, v_x_o, v_y_o, 0 : 16, 0 : 16], data_im2col_global_shared_wmma_matrix_a[v_n, v_x_o, v_k_o, 0 : 16, 0 : 16], weight_flatten_global_shared_wmma_matrix_b[v_k_o, v_y_o, 0 : 16, 0 : 16])
                                            T.writes(Conv_wmma_accumulator[v_n, v_x_o, v_y_o, 0 : 16, 0 : 16])
                                            ...
                                            C_3 = T.match_buffer(Conv_wmma_accumulator[v_n, v_x_o, v_y_o, 0 : 16, 0 : 16], [16, 16], dtype="float16", strides=[C_s0_3, C_s1_3], scope="wmma.accumulator", offset_factor=16)
                            ...
                            for ax0_0, ax1_0 in T.grid(1, 1):
                                with T.block("Conv_wmma.accumulator_o"):
                                    v0 = T.axis.spatial(1, 0)
                                    v1_o = T.axis.spatial(3025, x_0_0 + ax0_0)
                                    v2_o = T.axis.spatial(32, y_0_0_0 * 16 + y_0_0_1 + ax1_0)
                                    T.reads(Conv_wmma_accumulator[v0, v1_o, v2_o, 0 : 16, 0 : 16])
                                    T.writes(Conv[v0, v1_o * 16 : v1_o * 16 + 16, v2_o * 16 : v2_o * 16 + 16])
                                    A_4 = T.match_buffer(Conv_wmma_accumulator[v0, v1_o, v2_o, 0 : 16, 0 : 16], [16, 16], dtype="float16", strides=[A_s0_3, A_s1_3], scope="wmma.accumulator", offset_factor=16)
                                    C_4 = T.match_buffer(Conv[v0, v1_o * 16 : v1_o * 16 + 16, v2_o * 16 : v2_o * 16 + 16], [16, 16], dtype="float16", strides=[C_s0_4, C_s1_4], offset_factor=16)
