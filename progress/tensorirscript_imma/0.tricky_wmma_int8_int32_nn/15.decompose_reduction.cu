                  ...
                  for (i_init: int32, 0, 16) {
                    for (j_init: int32, 0, 16) {
                      block([1024, 1024, 16, 16], "B_init") as [vii, vjj, vi, vj] {
                        ...
                        bind(vjj, (((jj_2_init + (jj_0_0*64)) + (jj_0_1*4)) + jj_1))
                        bind(vi, i_init)
                        bind(vj, j_init)
                        tir.reads([])
                        ...
                        for (j: int32, 0, 16) {
                          for (k: int32, 0, 16) {
                            block([1024, 1024, tir.reduce_axis(0, 1024), 16, 16, tir.reduce_axis(0, 16)], "B_update") as [vii_1, vjj_1, vkk, vi_1, vj_1, vk] {
                              ...
                              C_wmma.accumulator[vii_1, vjj_1, vi_1, vj_1] = (C_wmma.accumulator[vii_1, vjj_1, vi_1, vj_1] + (cast(int32, A_shared_wmma.matrix_a[vii_1, vkk, vi_1, vk])*cast(int32, B_shared_wmma.matrix_b[vkk, vjj_1, vk, vj_1])))
                          }
                        }
                      }
                    }
