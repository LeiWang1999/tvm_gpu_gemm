    ...
    C_wmma.accumulator = alloc_buffer(int32[1024, 1024, 16, 16])
     {
      for (ax0: int32, 0, 1024) {
        for (ax1: int32, 0, 1024) {
          for (ax2: int32, 0, 16) {
            for (ax3: int32, 0, 16) {
              block([1024, 1024, 16, 16], "B_shared") as [v0, v1, v2, v3] {
                bind(v0, ax0)
                bind(v1, ax1)
                bind(v2, ax2)
                bind(v3, ax3)
        ...
        for (ax1_1: int32, 0, 1024) {
          for (ax2_1: int32, 0, 16) {
            for (ax3_1: int32, 0, 16) {
              block([1024, 1024, 16, 16], "A_shared") as [v0_1, v1_1, v2_1, v3_1] {
                bind(v0_1, ax0_1)
                bind(v1_1, ax1_1)
                bind(v2_1, ax2_1)
                bind(v3_1, ax3_1)
      ...
      for (ii_0: int32, 0, 64) {
        for (jj_0_0: int32, 0, 16) {
          for (jj_0_1: int32, 0, 16) {
            for (ii_1: int32, 0, 1) {
              for (jj_1: int32, 0, 4) {
                          ...
                          for (j: int32, 0, 16) {
                            for (k: int32, 0, 16) {
                              block([1024, 1024, tir.reduce_axis(0, 1024), 16, 16, tir.reduce_axis(0, 16)], "B") as [vii, vjj, vkk, vi, vj, vk] {
                                bind(vii, (((ii_0*16) + (ii_1*16)) + ii_2))
                                bind(vjj, (((jj_2 + (jj_0_0*64)) + (jj_0_1*4)) + jj_1))
                                ...
                                C_wmma.accumulator[vii, vjj, vi, vj] = (C_wmma.accumulator[vii, vjj, vi, vj] + (cast(int32, A_shared_wmma.matrix_a[vii, vkk, vi, vk])*cast(int32, B_shared_wmma.matrix_b[vkk, vjj, vk, vj])))
                            }
                          }
                        }
                      }
