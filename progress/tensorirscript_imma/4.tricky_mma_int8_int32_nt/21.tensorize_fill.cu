                ...
                for (ii_2: int32, 0, 4) {
                  for (jj_2: int32, 0, 4) {
                    for (i: int32, 0, 16) {
                      for (j: int32, 0, 16) {
                        for (k: int32, 0, 32) {
                          block([1024, 1024, tir.reduce_axis(0, 512), 16, 16, tir.reduce_axis(0, 32)], "B_update") as [vii_1, vjj_1, vkk, vi, vj, vk] {
                            ...
                            bind(vj, j)
                            bind(vk, k)
                            tir.reads([C_warp[vii_1, vjj_1, ((floormod(vi, 8)*4) + floordiv(floormod(vj, 8), 2)), (((floordiv(vj, 8)*4) + (floordiv(vi, 8)*2)) + floormod(vj, 2))], A_shared_warp[vii_1, vkk, ((vi*2) + floordiv(vk, 16)), floormod(vk, 16)], B_shared_warp[vjj_1, vkk, ((vj*2) + floordiv(vk, 16)), floormod(vk, 16)]])
