                      ...
                      for (kw: int32, 0, 3) {
                        for (ii_1: int32, 0, 16) {
                          block([16, 14, 14, 32, 16, 16, tir.reduce_axis(0, 16), tir.reduce_axis(0, 3), tir.reduce_axis(0, 3), tir.reduce_axis(0, 16)], "Conv") as [v_n_1, v_h_1, v_w_1, v_o, v_nn_1, v_oo, v_ic, v_kh, v_kw, v_ii_1] {
                            ...
                            bind(v_kw, kw)
                            bind(v_ii_1, ii_1)
                            tir.reads([Apad_shared_wmma.matrix_a[v_n_1, (v_h_1 + v_kh), (v_w_1 + v_kw), v_ic, v_nn_1, v_ii_1], W_shared_wmma.matrix_b[v_kh, v_kw, v_ic, v_o, v_ii_1, v_oo]])
                            tir.writes([Conv_wmma.accumulator[v_n_1, v_h_1, v_w_1, v_o, v_nn_1, v_oo]])
                            with init() {
                              Conv_wmma.accumulator[v_n_1, v_h_1, v_w_1, v_o, v_nn_1, v_oo] = 0f16
                            }
                            Conv_wmma.accumulator[v_n_1, v_h_1, v_w_1, v_o, v_nn_1, v_oo] = (Conv_wmma.accumulator[v_n_1, v_h_1, v_w_1, v_o, v_nn_1, v_oo] + (Apad_shared_wmma.matrix_a[v_n_1, (v_h_1 + v_kh), (v_w_1 + v_kw), v_ic, v_nn_1, v_ii_1]*W_shared_wmma.matrix_b[v_kh, v_kw, v_ic, v_o, v_ii_1, v_oo]))
                        }
                      }
                    }
                  }
