...
def func(A: T.Buffer[(256, 256), "float16"], B: T.Buffer[(256, 256), "float16"], C: T.Buffer[(256, 256), "float16"]):
    ...
    A_shared_warp = T.alloc_buffer([16, 16, 32, 8], dtype="float16", scope="warp")
    B_shared_warp = T.alloc_buffer([16, 16, 32, 8], dtype="float16", scope="warp")
    C_warp = T.alloc_buffer([16, 16, 32, 8], dtype="float16", scope="warp")
                ...
                for j_1_0 in T.thread_binding(4, thread="threadIdx.y"):
                    for i_1_1_0_init, j_1_1_0_init in T.grid(8, 2):
                        with T.block("B_init_o"):
                            vi_o = T.axis.spatial(16, i_1_0 * 8 + i_1_1_0_init)
                            vj_o = T.axis.spatial(16, j_0 * 8 + j_1_0 * 2 + j_1_1_0_init)
                            T.reads()
                    ...
                    for k_0 in T.serial(8):
                        ...
                        for i_1_1_0, j_1_1_0, k_1_0 in T.grid(8, 2, 2):
                            with T.block("A_shared_warp_o"):
                                v0_o = T.axis.spatial(16, i_1_0 * 8 + i_1_1_0)
                                v1_o = T.axis.spatial(16, k_0 * 2 + k_1_0)
                            ...
                            with T.block("B_shared_warp_o"):
                                v0_o = T.axis.spatial(16, j_0 * 8 + j_1_0 * 2 + j_1_1_0)
                                v1_o = T.axis.spatial(16, k_0 * 2 + k_1_0)
                                ...
                                T.launch_thread(tx, 32)
                                T.ptx_ldmatrix(False, 4, ".b16", warp_1.data, warp_1.elem_offset + 8 * tx, T.tvm_access_ptr(T.type_annotation(dtype="float16"), shared_1.data, shared_1.elem_offset, shared_s0_1 * 16, 1, dtype="handle"), shared_s0_1 * 8 * (tx // 16) + shared_s0_1 * (tx % 8) + 8 * (tx % 16 // 8), dtype="float16")
                            with T.block("B_update_o"):
                                vi_o = T.axis.spatial(16, i_1_0 * 8 + i_1_1_0)
                                vj_o = T.axis.spatial(16, j_0 * 8 + j_1_0 * 2 + j_1_1_0)
                                vk_o = T.axis.reduce(16, k_0 * 2 + k_1_0)
                    ...
                    for ax0_0, ax1_0 in T.grid(8, 2):
                        with T.block("C_warp_o"):
                            v0_o = T.axis.spatial(16, i_1_0 * 8 + ax0_0)
                            v1_o = T.axis.spatial(16, j_0 * 8 + j_1_0 * 2 + ax1_0)
