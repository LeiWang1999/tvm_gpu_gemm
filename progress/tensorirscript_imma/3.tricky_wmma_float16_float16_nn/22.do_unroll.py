        ...
        for jj_0 in T.thread_binding(64, thread="blockIdx.x"):
            for ii_1 in T.thread_binding(4, thread="threadIdx.y"):
                for jj_1 in T.thread_binding(4, thread="threadIdx.z"):
                    for ii_2_init in T.serial(2, annotations={"thread_rasterization":1}):
                        for jj_2_init in T.serial(4):
                                    ...
                                    for ax0_ax1_ax2_ax3_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                                        for ax0_ax1_ax2_ax3_fused_4 in T.vectorized(8):
                                            with T.block("A_shared"):
                                                v0 = T.axis.spatial(1024, ii_0 * 8 + (ax0_ax1_ax2_ax3_fused_0 * 1024 + ax0_ax1_ax2_ax3_fused_1 * 256 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) // 512)
                                    ...
                                    for ax0_ax1_ax2_ax3_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                                        for ax0_ax1_ax2_ax3_fused_4 in T.vectorized(8):
                                            with T.block("B_shared"):
                                                v0 = T.axis.spatial(1024, kk_0 * 2 + (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 512 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) // 4096)
                                                v1 = T.axis.spatial(1024, jj_0 * 16 + (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 512 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) % 4096 // 256)
                                                v2 = T.axis.spatial(16, (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 512 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) % 256 // 16)
                                                v3 = T.axis.spatial(16, (ax0_ax1_ax2_ax3_fused_0 * 2048 + ax0_ax1_ax2_ax3_fused_1 * 512 + ax0_ax1_ax2_ax3_fused_2 * 256 + ax0_ax1_ax2_ax3_fused_3 * 8 + ax0_ax1_ax2_ax3_fused_4) % 16)
                        ...
                        for kk_1 in T.serial(2):
                            ...
                            for ax0 in T.serial(4):
                                with T.block("B_shared_wmma.matrix_b_o"):
                                    ...
                                    v1 = T.axis.spatial(1024, jj_0 * 16 + jj_1 * 4 + ax0)
                                    v2_o = T.axis.spatial(1, 0)
                                    v3_o = T.axis.spatial(1, 0)
                                    ...
                                    T.tvm_load_matrix_sync(C_3.data, 16, 16, 16, C_3.elem_offset // C_s0_2 // 16 * (C_s0_2 // 16) + C_3.elem_offset % C_s0_2 // 16, T.tvm_access_ptr(T.type_annotation(dtype="float16"), A_2.data, A_2.elem_offset, A_s0_1 * 16, 1, dtype="handle"), A_s0_1, "row_major", dtype="handle")
                            for ii_2, jj_2 in T.grid(2, 4):
                                with T.block("B_update_o"):
                                    ...
                                    T.reads(C_wmma_accumulator[vii, vjj, 0 : 16, 0 : 16], A_shared_wmma_matrix_a[vii, vkk, 0 : 16, 0 : 16], B_shared_wmma_matrix_b[vkk, vjj, 0 : 16, 0 : 16])
                                    T.writes(C_wmma_accumulator[vii, vjj, 0 : 16, 0 : 16])
                                    ...
                                    B_1 = T.match_buffer(B_shared_wmma_matrix_b[vkk, vjj, 0 : 16, 0 : 16], [16, 16], dtype="float16", strides=[B_s0, B_s1], scope="wmma.matrix_b", offset_factor=16)
