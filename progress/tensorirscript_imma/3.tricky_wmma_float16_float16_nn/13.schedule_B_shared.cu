                      ...
                      for (j: int32, 0, 16) {
                        for (k: int32, 0, 16) {
                          block([1024, 1024, tir.reduce_axis(0, 1024), 16, 16, tir.reduce_axis(0, 16)], "B") as [vii, vjj, vkk, vi, vj, vk] {
                            bind(vii, (((ii_0*8) + (ii_1*2)) + ii_2))
                            bind(vjj, (((jj_0*16) + (jj_1*4)) + jj_2))
                            bind(vkk, ((kk_0*2) + kk_1))
                            bind(vi, i)
                            bind(vj, j)
                            bind(vk, k)
                            tir.reads([A_shared_wmma.matrix_a[vii, vkk, vi, vk], B_shared_wmma.matrix_b[vkk, vjj, vk, vj]])
                            tir.writes([C_wmma.accumulator[vii, vjj, vi, vj]])
                            with init() {
                              C_wmma.accumulator[vii, vjj, vi, vj] = 0f32
                            }
                            C_wmma.accumulator[vii, vjj, vi, vj] = (C_wmma.accumulator[vii, vjj, vi, vj] + (A_shared_wmma.matrix_a[vii, vkk, vi, vk]*B_shared_wmma.matrix_b[vkk, vjj, vk, vj]))
                        }
                      }
                    }
                  }
