...
def func(A: T.Buffer[(1, 56, 56, 64), "float16"], W: T.Buffer[(3, 3, 64, 64), "float16"], Conv: T.Buffer[(1, 3136, 64), "float16"]):
    ...
    for n, x, y in T.grid(1, 3136, 576):
        with T.block("data_im2col"):
            ...
            T.reads(A[v_n, v_y // 192 + v_x // 56 - 1, v_y % 192 // 64 + v_x % 56 - 1, v_y % 64])
            T.writes(data_im2col[v_n, v_x, v_y])
            data_im2col[v_n, v_x, v_y] = T.if_then_else(1 <= 1 * (v_x // 56) + 1 * (v_y // 64 // 3) and 1 * (v_x // 56) + 1 * (v_y // 64 // 3) < 57 and 1 <= 1 * (v_x % 56) + 1 * (v_y // 64 % 3) and 1 * (v_x % 56) + 1 * (v_y // 64 % 3) < 57, A[v_n, 1 * (v_x // 56) + 1 * (v_y // 64 // 3) - 1, 1 * (v_x % 56) + 1 * (v_y // 64 % 3) - 1, v_y % 64], T.float16(0), dtype="float16")
