...
def func(A: T.Buffer[(1, 56, 56, 64), "float16"], W: T.Buffer[(3, 3, 64, 64), "float16"], Conv: T.Buffer[(1, 3136, 64), "float16"]):
    ...
    for n, x_0, y_0, k_0, x_1, y_1, k_1 in T.grid(1, 196, 4, 36, 16, 16, 16):
        with T.block("Conv"):
            v_n = T.axis.spatial(1, n)
            v_x = T.axis.spatial(3136, x_0 * 16 + x_1)
            v_y = T.axis.spatial(64, y_0 * 16 + y_1)
            v_k = T.axis.reduce(576, k_0 * 16 + k_1)
