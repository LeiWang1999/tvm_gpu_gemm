            ...
            for y_0_0 in T.thread_binding(2, thread="blockIdx.x"):
                for x_0_1 in T.thread_binding(1, thread="threadIdx.y"):
                    for y_0_1 in T.thread_binding(1, thread="threadIdx.z"):
                        for x_0_2_init, y_0_2_init, x_1_init, y_1_init in T.grid(1, 2, 16, 16):
                            with T.block("Conv_init"):
                                v_n = T.axis.spatial(1, n)
                                v_x = T.axis.spatial(3136, x_0_0 * 16 + x_0_1 * 16 + x_0_2_init * 16 + x_1_init)
                                v_y = T.axis.spatial(64, y_0_0 * 32 + y_0_1 * 32 + y_0_2_init * 16 + y_1_init)
                                T.reads()
                            ...
                            for k_0_1 in T.serial(1):
                                for ax0_0, ax1_0, ax0_1, ax1_1 in T.grid(1, 1, 16, 16):
                                    with T.block("data_im2colPad_shared_wmma.matrix_a"):
                                        v0 = T.axis.spatial(1, 0)
                                        v1 = T.axis.spatial(3136, x_0_0 * 16 + ax0_0 * 16 + ax0_1)
                                        v2 = T.axis.spatial(576, k_0_0 * 16 + ax1_0 * 16 + ax1_1)
                                ...
                                for ax0_0, ax1_0, ax0_1, ax1_1 in T.grid(1, 2, 16, 16):
                                    with T.block("weight_flattenPad_shared_wmma.matrix_b"):
                                        v0 = T.axis.spatial(576, k_0_0 * 16 + ax0_0 * 16 + ax0_1)
                                        v1 = T.axis.spatial(64, y_0_0 * 32 + ax1_0 * 16 + ax1_1)
                                ...
                                for x_0_2, y_0_2, x_1, y_1, k_1 in T.grid(1, 2, 16, 16, 16):
                                    with T.block("Conv_update"):
                                        v_n = T.axis.spatial(1, n)
                                        v_x = T.axis.spatial(3136, x_0_0 * 16 + x_0_1 * 16 + x_0_2 * 16 + x_1)
                                        v_y = T.axis.spatial(64, y_0_0 * 32 + y_0_1 * 32 + y_0_2 * 16 + y_1)
                                        v_k = T.axis.reduce(576, k_0_0 * 16 + k_0_1 * 16 + k_1)
                        ...
                        for ax0_0 in T.serial(1):
                            for ax1_0, ax0_1, ax1_1 in T.grid(2, 16, 16):
                                with T.block("CPad_shared_wmma.accumulator"):
                                    v0 = T.axis.spatial(1, 0)
                                    v1 = T.axis.spatial(3136, x_0_0 * 16 + ax0_0 * 16 + ax0_1)
                                    v2 = T.axis.spatial(64, y_0_0 * 32 + ax1_0 * 16 + ax1_1)
