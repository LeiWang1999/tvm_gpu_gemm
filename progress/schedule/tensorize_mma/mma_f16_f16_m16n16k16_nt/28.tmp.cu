...
#endif
extern "C" __global__ void __launch_bounds__(32) main_kernel0(half* __restrict__ A, half* __restrict__ B, half* __restrict__ C) {
  __shared__ half A_shared[256];
  half A_shared_warp[8];
  half B_shared_warp[8];
  half C_warp[8];
  *(uint4*)(A_shared + (((int)threadIdx.x) * 8)) = *(uint4*)(A + (((((int)threadIdx.x) & 15) * 16) + ((((int)threadIdx.x) >> 4) * 8)));
  __syncthreads();

  {
    ...
    __asm__ __volatile__(
      ...
      : "=r"(addr)
      : "l"((void *)((&(A_shared[0])) + (((int)threadIdx.x) * 8)))
    );
    __asm__ __volatile__(
  ...
  __syncthreads();
  *(uint4*)(A_shared + (((int)threadIdx.x) * 8)) = *(uint4*)(B + (((((int)threadIdx.x) & 15) * 16) + ((((int)threadIdx.x) >> 4) * 8)));
  __syncthreads();
