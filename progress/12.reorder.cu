@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(B.shared.local: Pointer(local float32), float32, [65536]), storage_scope = local {
    for (ax0: int32, 0, 16384) {
      for (ax1: int32, 0, 4) {
        A.shared_1: Buffer(A.shared, float32, [65536], [], scope="shared")[((ax0*4) + ax1)] = A[(((((ax0*16384) + (blockIdx.x: int32*128)) + (vx: int32*64)) + (threadIdx.x: int32*4)) + ax1)]
      }
    }
    ...
    for (ax0_2: int32, 0, 16384) {
      for (ax1_2: int32, 0, 4) {
        A.shared_2: Buffer(A.shared, float32, [65536], [], scope="shared")[((ax0_2*4) + ax1_2)] = B[(((((ax0_2*16384) + (blockIdx.y: int32*128)) + (vy: int32*64)) + (threadIdx.y: int32*4)) + ax1_2)]
      }
    }
    ...
    attr [IterVar(threadIdx.x, (nullptr), "ThreadIndex", "threadIdx.x")] "thread_extent" = 16 {
      for (ii.c.init: int32, 0, 4) {
        for (jj.c.init: int32, 0, 4) {
          C.local_1: Buffer(C.local, float32, [16], [], scope="local", align=64)[((ii.c.init*4) + jj.c.init)] = 0f32
        }
      }
      for (k.outer: int32, 0, 1024) {
        for (k.inner.outer: int32, 0, 16) {
          for (ii.c: int32, 0, 4) {
            for (jj.c: int32, 0, 4) {
              let cse_var_4: int32 = ((ii.c*4) + jj.c)
              let cse_var_3: int32 = ((k.outer*64) + (k.inner.outer*4))
      ...
      for (ii.inner.inner.inner: int32, 0, 4) {
        for (jj.inner.inner.inner: int32, 0, 4) {
          let cse_var_5: int32 = ((ii.inner.inner.inner*4) + jj.inner.inner.inner)
           {
            C[((((((blockIdx.y*2097152) + (threadIdx.y*65536)) + (ii.inner.inner.inner*16384)) + (blockIdx.x*128)) + (threadIdx.x*4)) + jj.inner.inner.inner)] = C.local_1[cse_var_5]
            C[(((((((blockIdx.y*2097152) + (threadIdx.y*65536)) + (ii.inner.inner.inner*16384)) + (blockIdx.x*128)) + (threadIdx.x*4)) + jj.inner.inner.inner) + 1048576)] = C.local_1[cse_var_5]
            C[(((((((blockIdx.y*2097152) + (threadIdx.y*65536)) + (ii.inner.inner.inner*16384)) + (blockIdx.x*128)) + (threadIdx.x*4)) + jj.inner.inner.inner) + 64)] = C.local_1[cse_var_5]
            C[(((((((blockIdx.y*2097152) + (threadIdx.y*65536)) + (ii.inner.inner.inner*16384)) + (blockIdx.x*128)) + (threadIdx.x*4)) + jj.inner.inner.inner) + 1048640)] = C.local_1[cse_var_5]
          }
        }
      }
    }
