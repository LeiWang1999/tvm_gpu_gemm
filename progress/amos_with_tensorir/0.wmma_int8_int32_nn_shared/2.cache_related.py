...
def func(A: T.Buffer[(16384, 16384), "int8"], B: T.Buffer[(16384, 16384), "int8"], C: T.Buffer[(16384, 16384), "int32"]):
    ...
    C_shared_wmma_accumulator = T.alloc_buffer([16384, 16384], dtype="int32", scope="wmma.accumulator")
    ...
    for i, j, k in T.grid(16384, 16384, 16384):
        with T.block("B"):
            ...
            T.writes(C_shared_wmma_accumulator[vi, vj])
            with T.init():
                C_shared_wmma_accumulator[vi, vj] = 0
            C_shared_wmma_accumulator[vi, vj] = C_shared_wmma_accumulator[vi, vj] + T.Cast("int32", A_global_shared_wmma_matrix_a[vi, vk]) * T.Cast("int32", B_global_shared_wmma_matrix_b[vk, vj])
    for ax0, ax1 in T.grid(16384, 16384):
        with T.block("C_shared_wmma.accumulator"):
            v0, v1 = T.axis.remap("SS", [ax0, ax1])
            T.reads(C_shared_wmma_accumulator[v0, v1])
            T.writes(C_shared[v0, v1])
            C_shared[v0, v1] = C_shared_wmma_accumulator[v0, v1]
    for ax0, ax1 in T.grid(16384, 16384):
