...
@main = primfn(a: handle, b: handle, c: handle) -> ()
  ...
  block([], "root") {
    ...
    C_shared_wmma.accumulator = alloc_buffer(int32[16384, 16384])
     {
        ...
        for (j: int32, 0, 16384) {
          for (k: int32, 0, 16384) {
            block([16384, 16384, tir.reduce_axis(0, 16384)], "B") as [vi, vj, vk] {
              ...
              tir.writes([C_shared_wmma.accumulator[vi, vj]])
              with init() {
                C_shared_wmma.accumulator[vi, vj] = 0
              }
              C_shared_wmma.accumulator[vi, vj] = (C_shared_wmma.accumulator[vi, vj] + (cast(int32, A_global_shared_wmma.matrix_a[vi, vk])*cast(int32, B_global_shared_wmma.matrix_b[vk, vj])))
          }
        }
      }
      for (ax0_6: int32, 0, 16384) {
        for (ax1_6: int32, 0, 16384) {
          block([16384, 16384], "C_shared_wmma.accumulator") as [v0_6, v1_6] {
            bind(v0_6, ax0_6)
            bind(v1_6, ax1_6)
            tir.reads([C_shared_wmma.accumulator[v0_6, v1_6]])
            tir.writes([C_shared[v0_6, v1_6]])
            C_shared[v0_6, v1_6] = C_shared_wmma.accumulator[v0_6, v1_6]
        }
      }
