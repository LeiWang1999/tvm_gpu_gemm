...
extern "C" __global__ void __launch_bounds__(128) main_kernel2(signed char* __restrict__ A_global, signed char* __restrict__ B_global, int* __restrict__ C) {
  nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, int> C_shared_wmma_accumulator[16];
  __shared__ signed char A_global_shared[4096];
  ...
  __shared__ int C_shared[2048];
  for (int i_0_2_init = 0; i_0_2_init < 8; ++i_0_2_init) {
    for (int j_0_2_init = 0; j_0_2_init < 2; ++j_0_2_init) {
      nvcuda::wmma::fill_fragment(C_shared_wmma_accumulator[((i_0_2_init * 2) + j_0_2_init)], 0.000000e+00f);
    }
  }
  ...
  for (int k_0_0 = 0; k_0_0 < 512; ++k_0_0) {
    ...
    for (int ax0_0_ax1_0_ax0_1_ax1_1_fused_2 = 0; ax0_0_ax1_0_ax0_1_ax1_1_fused_2 < 2; ++ax0_0_ax1_0_ax0_1_ax1_1_fused_2) {
      *(int4*)(A_global_shared + (((((int)threadIdx.z) * 1024) + (ax0_0_ax1_0_ax0_1_ax1_1_fused_2 * 512)) + (((int)threadIdx.x) * 16))) = *(int4*)(A_global + (((((((int)blockIdx.y) * 2097152) + (((int)threadIdx.z) * 524288)) + (ax0_0_ax1_0_ax0_1_ax1_1_fused_2 * 262144)) + (k_0_0 * 512)) + (((int)threadIdx.x) * 16)));
    }
    for (int ax0_0_ax1_0_ax0_1_ax1_1_fused_2_1 = 0; ax0_0_ax1_0_ax0_1_ax1_1_fused_2_1 < 2; ++ax0_0_ax1_0_ax0_1_ax1_1_fused_2_1) {
      *(int4*)(B_global_shared + (((((int)threadIdx.z) * 1024) + (ax0_0_ax1_0_ax0_1_ax1_1_fused_2_1 * 512)) + (((int)threadIdx.x) * 16))) = *(int4*)(B_global + (((((k_0_0 * 524288) + ((((int)threadIdx.z) >> 1) * 262144)) + (((int)blockIdx.x) * 2048)) + (((((((int)threadIdx.z) * 4) + (ax0_0_ax1_0_ax0_1_ax1_1_fused_2_1 * 2)) + (((int)threadIdx.x) >> 4)) & 7) * 256)) + ((((int)threadIdx.x) & 15) * 16)));
    }
    __syncthreads();
    for (int k_0_1 = 0; k_0_1 < 2; ++k_0_1) {
      ...
      for (int ax1_0 = 0; ax1_0 < 2; ++ax1_0) {
        nvcuda::wmma::load_matrix_sync(B_global_shared_wmma_matrix_b[ax1_0], (&(B_global_shared[(((k_0_1 * 2048) + (((int)threadIdx.z) * 512)) + (ax1_0 * 256))])), 16);
      }
      for (int i_0_2 = 0; i_0_2 < 8; ++i_0_2) {
        for (int j_0_2 = 0; j_0_2 < 2; ++j_0_2) {
          nvcuda::wmma::mma_sync(C_shared_wmma_accumulator[((i_0_2 * 2) + j_0_2)], A_global_shared_wmma_matrix_a[i_0_2], B_global_shared_wmma_matrix_b[j_0_2], C_shared_wmma_accumulator[((i_0_2 * 2) + j_0_2)]);
        }
      }
    }
  }
  for (int ax0_0_1 = 0; ax0_0_1 < 8; ++ax0_0_1) {
    ...
    for (int ax1_0_1 = 0; ax1_0_1 < 2; ++ax1_0_1) {
      nvcuda::wmma::store_matrix_sync((&(C_shared[((((int)threadIdx.z) * 32) + (ax1_0_1 * 16))])), C_shared_wmma_accumulator[((ax0_0_1 * 2) + ax1_0_1)], 128, nvcuda::wmma::mem_row_major);
    }
    __syncthreads();
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0) {
      C[((((((((int)blockIdx.y) * 2097152) + (ax0_0_1 * 262144)) + (ax0_ax1_fused_0 * 65536)) + (((int)threadIdx.z) * 16416)) + (((int)blockIdx.x) * 128)) + ((int)threadIdx.x))] = C_shared[(((ax0_ax1_fused_0 * 512) + (((int)threadIdx.z) * 160)) + ((int)threadIdx.x))];
    }
  }
}
