...
@main = primfn(a: handle, b: handle, c: handle) -> ()
  ...
  block([], "root") {
    tir.reads([])
    tir.writes([])
    A_global = alloc_buffer(float16[16384, 16384])
    A_global_shared = alloc_buffer(float16[16384, 16384])
    A_global_shared_warp = alloc_buffer(float16[16384, 16384])
    B_global = alloc_buffer(float16[16384, 16384])
    B_global_shared = alloc_buffer(float16[16384, 16384])
    B_global_shared_warp = alloc_buffer(float16[16384, 16384])
        ...
        for (j: int32, 0, 16384) {
          for (k: int32, 0, 16384) {
            block([16384, 16384, tir.reduce_axis(0, 16384)], "B") as [vi, vj, vk] {
              ...
              bind(vj, j)
              bind(vk, k)
              tir.reads([A_global_shared_warp[vi, vk], B_global_shared_warp[vj, vk]])
              tir.writes([C_warp[vi, vj]])
              with init() {
                C_warp[vi, vj] = 0f16
              }
              C_warp[vi, vj] = (C_warp[vi, vj] + (A_global_shared_warp[vi, vk]*B_global_shared_warp[vj, vk]))
          }
        }
      }
