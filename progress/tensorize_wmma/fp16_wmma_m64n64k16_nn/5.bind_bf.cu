@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(B.shared: Pointer(shared float16), float16, [65536]), storage_scope = shared {
    for (ax0: int32, 0, 256) {
    ...
    for (ax0_1: int32, 0, 1024) {
      for (ax1_1: int32, 0, 64) {
        B.shared_1: Buffer(B.shared, float16, [65536], [], scope="shared")[((ax0_1*64) + ax1_1)] = B[(((ax0_1*1024) + (blockIdx.y: int32*64)) + ax1_1)]
      }
    }
          ...
          for (ax0.outer: int32, 0, 4) {
            for (ax0.inner: int32, 0, 32) {
              for (ax1.inner: int32, 0, 16) {
                A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, float16, [2048], [], scope="wmma.matrix_a")[(((ax0.outer*512) + (ax0.inner*16)) + ax1.inner)] = A.shared_1[((((((threadIdx.y*131072) + (ax0.outer*32768)) + (ax0.inner*1024)) + (rk.outer.outer*64)) + (rk.outer.inner*16)) + ax1.inner)]
              }
            }
          }
          for (ax1.outer: int32, 0, 2) {
            for (ax0.inner_1: int32, 0, 16) {
              for (ax1.inner_1: int32, 0, 8) {
                ...
                B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, float16, [256], [], scope="wmma.matrix_b")[(((ax0.inner_1*16) + cse_var_2) + ax1.inner_1)] = B.shared_1[((((((rk.outer.outer*4096) + (rk.outer.inner*1024)) + (ax0.inner_1*64)) + (threadIdx.z*16)) + cse_var_2) + ax1.inner_1)]
              }
            }
          }
