@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(A.shared: Pointer(shared float16), float16, [262144]), storage_scope = shared;
  allocate(B.shared: Pointer(shared float16), float16, [16384]), storage_scope = shared;
  allocate(B.shared.wmma.matrix_b: Pointer(wmma.matrix_b float16), float16, [16384]), storage_scope = wmma.matrix_b {
    for (ax0: int32, 0, 256) {
      for (ax1: int32, 0, 1024) {
        ...
        A.shared_1: Buffer(A.shared, float16, [262144], [], scope="shared")[(cse_var_1 + ax1)] = A[(((blockIdx.x: int32*262144) + cse_var_1) + ax1)]
      }
    }
    for (ax0_1: int32, 0, 1024) {
      for (ax1_1: int32, 0, 16) {
        B.shared_1: Buffer(B.shared, float16, [16384], [], scope="shared")[((ax0_1*16) + ax1_1)] = B[((((ax0_1*1024) + (blockIdx.y: int32*64)) + (threadIdx.z: int32*16)) + ax1_1)]
      }
    }
    for (ax0_2: int32, 0, 1024) {
      for (ax1_2: int32, 0, 16) {
        let cse_var_2: int32 = ((ax0_2*16) + ax1_2)
        B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, float16, [16384], [], scope="wmma.matrix_b")[cse_var_2] = B.shared_1[cse_var_2]
      }
    }
        ...
        for (rk.outer.inner: int32, 0, 4) {
          for (ax0_3: int32, 0, 128) {
            for (ax1_3: int32, 0, 16) {
              A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, float16, [2048], [], scope="wmma.matrix_a")[((ax0_3*16) + ax1_3)] = A.shared_1[(((((threadIdx.y*131072) + (ax0_3*1024)) + (rk.outer.outer*64)) + (rk.outer.inner*16)) + ax1_3)]
            }
          }
              ...
              for (ii.c.inner: int32, 0, 32) {
                for (jj.c.inner: int32, 0, 8) {
                  for (rk.inner: int32, 0, 16) {
                    ...
                    C.wmma.accumulator_1[cse_var_3] = (C.wmma.accumulator_1[cse_var_3] + (cast(float32, A.shared.wmma.matrix_a_1[(cse_var_4 + rk.inner)])*cast(float32, B.shared.wmma.matrix_b_1[(((((rk.outer.outer*1024) + (rk.outer.inner*256)) + (rk.inner*16)) + cse_var_5) + jj.c.inner)])))
                  }
                }
              }
            }
