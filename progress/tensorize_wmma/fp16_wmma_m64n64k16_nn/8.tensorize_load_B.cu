            ...
            for (ii.c.inner: int32, 0, 32) {
              for (jj.c.inner: int32, 0, 8) {
                for (rk.inner: int32, 0, 16) {
                  ...
                  C.wmma.accumulator_1[cse_var_3] = (C.wmma.accumulator_1[cse_var_3] + (cast(float32, A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, float16, [2048], [], scope="wmma.matrix_a")[(cse_var_4 + rk.inner)])*cast(float32, B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, float16, [256], [], scope="wmma.matrix_b")[(((rk.inner*16) + cse_var_5) + jj.c.inner)])))
                }
              }
            }
          }
