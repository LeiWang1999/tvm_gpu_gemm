    ...
    for (ii.c: int32, 0, 64) {
      for (jj.c: int32, 0, 64) {
        ...
        for (rk: int32, 0, 64) {
          ...
          C.wmma.accumulator_1[cse_var_5] = (C.wmma.accumulator_1[cse_var_5] + (cast(float32, A.shared.wmma.matrix_a_1[(cse_var_6 + rk)])*cast(float32, B.shared.wmma.matrix_b_1[((rk*64) + jj.c)])))
        }
      }
    }
      ...
      for (jj.inner.outer.inner: int32, 0, 4) {
        for (ii.inner.inner: int32, 0, 16) {
          for (jj.inner.inner: int32, 0, 16) {
            let cse_var_7: int32 = ((((ii.inner.outer.inner*1024) + (ii.inner.inner*64)) + (jj.inner.outer.inner*16)) + jj.inner.inner)
