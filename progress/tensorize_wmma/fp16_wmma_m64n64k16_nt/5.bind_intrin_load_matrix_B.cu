          ...
          for (ii.c.inner: int32, 0, 16) {
            for (jj.c.inner: int32, 0, 16) {
              for (rk.inner: int32, 0, 16) {
                let cse_var_1: int32 = ((((ii.c.outer*1024) + (ii.c.inner*64)) + (jj.c.outer*16)) + jj.c.inner)
                C.wmma.accumulator_1[cse_var_1] = (C.wmma.accumulator_1[cse_var_1] + (cast(float32, A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, float16, [1024], [], scope="wmma.matrix_a")[(((ii.c.outer*256) + (ii.c.inner*16)) + rk.inner)])*cast(float32, B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, float16, [1024], [], scope="wmma.matrix_b")[(((jj.c.outer*256) + (jj.c.inner*16)) + rk.inner)])))
              }
            }
          }
        }
      ...
      for (jj.inner.outer.inner: int32, 0, 4) {
        for (ii.inner.inner: int32, 0, 16) {
          for (jj.inner.inner: int32, 0, 16) {
            ...
            C[((((((blockIdx.y*16384) + (ii.inner.outer.inner*4096)) + (ii.inner.inner*256)) + (blockIdx.x*64)) + cse_var_2) + jj.inner.inner)] = C.wmma.accumulator_1[((((ii.inner.outer.inner*1024) + (ii.inner.inner*64)) + cse_var_2) + jj.inner.inner)]
          }
        }
      }
    }
