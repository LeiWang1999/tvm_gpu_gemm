  ...
  allocate(B.shared.wmma.matrix_b: Pointer(wmma.matrix_b float16), float16, [2048]), storage_scope = wmma.matrix_b {
    ...
    for (ax0_1: int32, 0, 64) {
      for (ax1_1: int32, 0, 32) {
        ...
        A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, float16, [2048], [], scope="wmma.matrix_a")[cse_var_2] = A.shared_1[cse_var_2]
      }
    }
    for (ax0_2: int32, 0, 64) {
      for (ax1_2: int32, 0, 32) {
        let cse_var_3: int32 = (ax0_2*32)
        A.shared_2: Buffer(A.shared, float16, [2048], [], scope="shared")[(cse_var_3 + ax1_2)] = B[(((blockIdx.x: int32*2048) + cse_var_3) + ax1_2)]
      }
    }
    for (ax0_3: int32, 0, 64) {
      for (ax1_3: int32, 0, 32) {
        let cse_var_4: int32 = ((ax0_3*32) + ax1_3)
        B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, float16, [2048], [], scope="wmma.matrix_b")[cse_var_4] = A.shared_2[cse_var_4]
      }
    }
            ...
            for (ii.c.inner: int32, 0, 16) {
              for (jj.c.inner: int32, 0, 16) {
                for (rk.inner: int32, 0, 16) {
                  let cse_var_6: int32 = (rk.outer.inner*16)
                  let cse_var_5: int32 = ((((ii.c.outer*1024) + (ii.c.inner*64)) + (jj.c.outer*16)) + jj.c.inner)
                  C.wmma.accumulator_1[cse_var_5] = (C.wmma.accumulator_1[cse_var_5] + (cast(float32, A.shared.wmma.matrix_a_1[((((ii.c.outer*512) + (ii.c.inner*32)) + cse_var_6) + rk.inner)])*cast(float32, B.shared.wmma.matrix_b_1[((((jj.c.outer*512) + (jj.c.inner*32)) + cse_var_6) + rk.inner)])))
                }
              }
            }
          }
        ...
        for (jj.inner.outer.inner: int32, 0, 4) {
          for (ii.inner.inner: int32, 0, 16) {
            for (jj.inner.inner: int32, 0, 16) {
              let cse_var_7: int32 = (jj.inner.outer.inner*16)
              C[((((((blockIdx.y*16384) + (ii.inner.outer.inner*4096)) + (ii.inner.inner*256)) + (blockIdx.x*64)) + cse_var_7) + jj.inner.inner)] = C.wmma.accumulator_1[((((ii.inner.outer.inner*1024) + (ii.inner.inner*64)) + cse_var_7) + jj.inner.inner)]
            }
          }
        }
      }
