      ...
      for (j_0: int32, 0, 64) "thread_binding" {
            ...
            for (i_1_1_0_init: int32, 0, 4) {
              for (j_1_1_0_init: int32, 0, 4) {
                block([1024, 1024], "B_init_o") as [vi_o, vj_o] {
                  bind(vi_o, (((i_0*8) + (i_1_0_init*4)) + i_1_1_0_init))
                  bind(vj_o, (((j_0*16) + (j_1_0_init*4)) + j_1_1_0_init))
                  tir.reads([])
        ...
        for (k_0: int32, 0, 256) {
          ...
          for (ax0_0_1: int32, 0, 4) "thread_binding" {
            for (ax0_1: int32, 0, 32) "thread_binding" {
              for (ax1_0_1: int32, 0, 2) "thread_binding" {
                for (ax1_1: int32, 0, 32) {
                  block([16384, tir.reduce_axis(0, 16384)], "Pre_compute_A_update") as [vi_1, vk] {
                    bind(vi_1, (((i_0*128) + (ax0_0_1*32)) + ax0_1))
                    bind(vk, (((k_0*64) + (ax1_0_1*32)) + ax1_1))
                    tir.reads([PA[vi_1], A_shared[vi_1, vk]])
                    tir.writes([PA[vi_1]])
                    PA[vi_1] = (PA[vi_1] + (1*cast(int32, A_shared[vi_1, vk])))
                }
              }
            }
          }
