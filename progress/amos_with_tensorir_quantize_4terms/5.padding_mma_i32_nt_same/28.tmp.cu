...
extern "C" __global__ void __launch_bounds__(256) main_kernel0(signed char* __restrict__ A, signed char* __restrict__ B, int* __restrict__ C) {
  ...
  PA[((((int)threadIdx.y) * 32) + ((int)threadIdx.x))] = 0;
...
;
    ...
    for (int ax1_1 = 0; ax1_1 < 32; ++ax1_1) {
      __syncthreads();
      PA[((((int)threadIdx.y) * 32) + ((int)threadIdx.x))] = (PA[((((int)threadIdx.y) * 32) + ((int)threadIdx.x))] + ((int)A_shared[((((((int)threadIdx.y) * 2048) + (((int)threadIdx.x) * 64)) + (((int)threadIdx.z) * 32)) + ax1_1)]));
    }
  }
}
