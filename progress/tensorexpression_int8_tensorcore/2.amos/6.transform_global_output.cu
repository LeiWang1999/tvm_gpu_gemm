@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(C.local.global: Pointer(global int32), int32, [268435456]), storage_scope = global {
    ...
    for (axis0_7: int32, 0, 1024) {
      for (axis1_7: int32, 0, 1024) {
        for (axis2_7: int32, 0, 16) {
          for (axis3_7: int32, 0, 16) {
            let cse_var_11: int32 = ((((axis0_7*262144) + (axis1_7*256)) + (axis2_7*16)) + axis3_7)
            C.local.global_1: Buffer(C.local.global, int32, [268435456], [])[cse_var_11] = C.local_1[cse_var_11]
          }
        }
      }
    }
