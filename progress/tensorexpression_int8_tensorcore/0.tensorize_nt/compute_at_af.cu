        ...
        for (rk.outer.inner: int32, 0, 4) {
          for (ax0_3: int32, 0, 32) {
            for (ax1_3: int32, 0, 16) {
              A.global.shared.wmma.matrix_a_1: Buffer(A.global.shared.wmma.matrix_a, int8, [512], [], scope="wmma.matrix_a")[((ax0_3*16) + ax1_3)] = A.global.shared_1[(((((threadIdx.y*524288) + (ax0_3*16384)) + (rk.outer.outer*64)) + (rk.outer.inner*16)) + ax1_3)]
            }
          }
