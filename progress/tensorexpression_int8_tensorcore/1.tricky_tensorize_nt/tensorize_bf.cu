  ...
  attr [IterVar(threadIdx.z: int32, (nullptr), "ThreadIndex", "threadIdx.z")] "thread_extent" = 2 {
    ...
    for (k1.outer: int32, 0, 256) {
      ...
      for (k1.inner: int32, 0, 4) {
        ...
        for (ax0_1: int32, 0, 8) {
          @tir.tvm_load_matrix_sync(B.shared.wmma.matrix_b, 16, 16, 16, ax0_1, @tir.tvm_access_ptr(@tir.type_annotation(, dtype=int8), B.shared, (((threadIdx.z*8192) + (ax0_1*1024)) + (k1.inner*256)), 256, 1, dtype=handle), 16, "col_major", dtype=handle)
        }
        for (i.c: int32, 0, 2) {
          for (j.c: int32, 0, 8) {
            for (ii.c: int32, 0, 16) {
              for (jj.c: int32, 0, 16) {
                for (k2: int32, 0, 16) {
                  ...
                  let cse_var_2: int32 = ((((i.c*2048) + cse_var_4) + cse_var_3) + jj.c)
                  C.wmma.accumulator_1[cse_var_2] = (C.wmma.accumulator_1[cse_var_2] + (cast(int32, A.shared.wmma.matrix_a_1: Buffer(A.shared.wmma.matrix_a, int8, [512], [], scope="wmma.matrix_a")[(((i.c*256) + cse_var_3) + k2)])*cast(int32, B.shared.wmma.matrix_b_1: Buffer(B.shared.wmma.matrix_b, int8, [2048], [], scope="wmma.matrix_b")[((cse_var_4 + (jj.c*16)) + k2)])))
                }
              }
            }
          }
      ...
      for (j.inner: int32, 0, 8) {
        for (ii: int32, 0, 16) {
          for (jj: int32, 0, 16) {
            ...
            C[((((((((blockIdx.x*1048576) + (threadIdx.y*524288)) + (i.inner*262144)) + (blockIdx.y*4096)) + (threadIdx.z*2048)) + cse_var_6) + cse_var_5) + jj)] = C.wmma.accumulator_1[((((i.inner*2048) + cse_var_6) + cse_var_5) + jj)]
          }
        }
      }
    }
