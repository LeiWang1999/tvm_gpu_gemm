                ...
                for j_1_0 in T.thread_binding(4, thread="threadIdx.y"):
                    for i_1_1_0_init, j_1_1_0_init in T.grid(4, 4):
                        with T.block("B_init_o"):
                            ...
                            T.evaluate(T.mma_fill(8, C_warp_1.data, C_warp_1.elem_offset, dtype="int32"))
                    for k_0 in T.serial(256):
                        ...
                        for i_1_1_0, j_1_1_0, k_1_0 in T.grid(4, 4, 2):
                            with T.block("A_shared_warp_o"):
                                ...
                                T.launch_thread(tx, 32)
                                T.evaluate(T.ptx_ldmatrix(False, 4, ".b16", warp.data, warp.elem_offset + 16 * tx, T.tvm_access_ptr(T.type_annotation(dtype="int8"), shared.data, shared.elem_offset, shared_s0 * 16, 1, dtype="handle"), shared_s0 * (tx % 16) + 16 * (tx // 16), dtype="int8"))
                            with T.block("B_shared_warp_o"):
                                ...
                                T.launch_thread(tx, 32)
                                T.evaluate(T.ptx_ldmatrix(False, 4, ".b16", warp_1.data, warp_1.elem_offset + 16 * tx, T.tvm_access_ptr(T.type_annotation(dtype="int8"), shared_1.data, shared_1.elem_offset, shared_s0_1 * 16, 1, dtype="handle"), shared_s0_1 * 8 * (tx // 16) + tx % 8 * shared_s0_1 + 16 * (tx % 16 // 8), dtype="int8"))
                            with T.block("B_update_o"):
                                ...
                                T.launch_thread(tx, 32)
                                T.evaluate(T.ptx_mma("m16n8k32", "row", "col", "int8", "int8", "int32", A_1.data, A_1.elem_offset + tx * 16, B_1.data, B_1.elem_offset + tx * 16, C_1.data, C_1.elem_offset + tx * 8, False, dtype="int32"))
                                T.evaluate(T.ptx_mma("m16n8k32", "row", "col", "int8", "int8", "int32", A_1.data, A_1.elem_offset + tx * 16, B_1.data, B_1.elem_offset + tx * 16 + T.FloorDiv(16, 2), C_1.data, C_1.elem_offset + tx * 8 + T.FloorDiv(8, 2), False, dtype="int32"))
                    for ax0_0, ax1_0 in T.grid(4, 4):
                        with T.block("C_warp_o"):
                            ...
                            T.launch_thread(tx, 32)
                            T.evaluate(T.mma_store(16, 16, T.tvm_access_ptr(T.type_annotation(dtype="int32"), C_2.data, C_2.elem_offset, C_s0 * 16, 2, dtype="handle"), C_warp_2.data, C_warp_2.elem_offset, C_s0, dtype="int32"))
