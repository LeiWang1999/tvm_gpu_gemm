...
extern "C" __global__ void __launch_bounds__(256) main_kernel0(signed char* __restrict__ A, signed char* __restrict__ B, int* __restrict__ C) {
  int C_warp[128];
  __shared__ signed char A_shared[10240];
  __shared__ signed char B_shared[20480];
...
;
  ...
  for (int k_0 = 0; k_0 < 256; ++k_0) {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0) {
      *(int4*)(A_shared + ((((ax0_ax1_fused_0 * 2560) + (((int)threadIdx.y) * 640)) + ((((int)threadIdx.x) >> 2) * 80)) + ((((int)threadIdx.x) & 3) * 16))) = *(int4*)(A + ((((((((int)blockIdx.y) * 2097152) + (ax0_ax1_fused_0 * 524288)) + (((int)threadIdx.y) * 131072)) + ((((int)threadIdx.x) >> 2) * 16384)) + (k_0 * 64)) + ((((int)threadIdx.x) & 3) * 16)));
    }
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 8; ++ax0_ax1_fused_0_1) {
      *(int4*)(B_shared + ((((ax0_ax1_fused_0_1 * 2560) + (((int)threadIdx.y) * 640)) + ((((int)threadIdx.x) >> 2) * 80)) + ((((int)threadIdx.x) & 3) * 16))) = *(int4*)(B + ((((((((int)blockIdx.x) * 4194304) + (ax0_ax1_fused_0_1 * 524288)) + (((int)threadIdx.y) * 131072)) + ((((int)threadIdx.x) >> 2) * 16384)) + (k_0 * 64)) + ((((int)threadIdx.x) & 3) * 16)));
    }
    __syncthreads();
...

  {
    ...
    __asm__ __volatile__(
      ...
      : "=r"(addr)
      : "l"((void *)((&(A_shared[(((((int)threadIdx.z) * 5120) + (i_1_1_0 * 1280)) + (k_1_0 * 32))])) + (((((int)threadIdx.x) & 15) * 80) + ((((int)threadIdx.x) >> 4) * 16))))
    );
    __asm__ __volatile__(
...

  {
    ...
    __asm__ __volatile__(
      "{ .reg .u64 addr; cvta.to.shared.u64 addr, %1; cvt.u32.u64 %0, addr; }\n"
      : "=r"(addr)
      : "l"((void *)((&(B_shared[(((((int)threadIdx.y) * 5120) + (j_1_1_0 * 1280)) + (k_1_0 * 32))])) + ((((((int)threadIdx.x) >> 4) * 640) + ((((int)threadIdx.x) & 7) * 80)) + (((((int)threadIdx.x) & 15) >> 3) * 16))))
    );
    __asm__ __volatile__(
      "ldmatrix.sync.aligned.m8n8.x4.shared.b16"
      "{%0, %1, %2, %3}, [%4];\n"
