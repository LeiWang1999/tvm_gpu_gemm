            ...
            for i_1_0 in T.thread_binding(2, thread="threadIdx.z"):
                for j_1_0 in T.thread_binding(4, thread="threadIdx.y"):
                    for k_0 in T.serial(256):
                        ...
                        for k_1_0, i_1_1_0, j_1_1_0, i_1_1_1, j_1_1_1, k_1_1 in T.grid(2, 4, 4, 16, 16, 32):
                            with T.block("B"):
                                ...
                                T.reads(A_shared[vi // 8, vk // 64, vi % 8, vk % 64], B_shared[vj // 8, vk // 64, vj % 8, vk % 64])
                                T.writes(C[vi, vj])
                                with T.init():
                                    C[vi, vj] = 0
                                C[vi, vj] = C[vi, vj] + T.Cast("int32", A_shared[vi // 8, vk // 64, vi % 8, vk % 64]) * T.Cast("int32", B_shared[vj // 8, vk // 64, vj % 8, vk % 64])
