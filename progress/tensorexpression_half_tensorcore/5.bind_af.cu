      ...
      attr [IterVar(threadIdx.y: int32, (nullptr), "ThreadIndex", "threadIdx.y")] "thread_extent" = 1 {
          ...
          for (ii.c.inner: int32, 0, 16) {
            for (jj.c.inner: int32, 0, 16) {
              for (rk.inner: int32, 0, 16) {
                ...
                C.wmma.accumulator_1[cse_var_4] = (C.wmma.accumulator_1[cse_var_4] + (cast(float32, A.shared.wmma.matrix_a_1[(cse_var_5 + rk.inner)])*cast(float32, B.shared.wmma.matrix_b_1[(((jj.c.inner*64) + (rk.outer.outer*16)) + rk.inner)])))
              }
            }
          }
        }
        for (ax0.inner_1: int32, 0, 16) {
          for (ax1.inner_1: int32, 0, 16) {
            let cse_var_6: int32 = ((ax0.inner_1*16) + ax1.inner_1)
