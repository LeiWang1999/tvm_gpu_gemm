@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  attr [IterVar(blockIdx.x: int32, (nullptr), "ThreadIndex", "blockIdx.x")] "thread_extent" = 4 {
    ...
    attr [IterVar(threadIdx.x, (nullptr), "ThreadIndex", "threadIdx.x")] "thread_extent" = 32;
    C[ramp(((((blockIdx.y*1024) + (floordiv(threadIdx.x, 2)*64)) + (blockIdx.x*16)) + (floormod(threadIdx.x, 2)*8)), 1, 8)] = C.wmma.accumulator.shared_1: Buffer(C.wmma.accumulator.shared, float32, [256], [], scope="shared")[ramp((threadIdx.x*8), 1, 8)]
  }
}
