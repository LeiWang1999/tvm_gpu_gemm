@main = primfn(A_1: handle, B_1: handle, C_1: handle) -> ()
  ...
  allocate(C.wmma.accumulator.shared: Pointer(shared float32), float32, [4096]), storage_scope = shared {
    for (ax0: int32, 0, 64) {
      for (ax1: int32, 0, 64) {
    ...
    for (ii.c: int32, 0, 64) {
      for (jj.c: int32, 0, 64) {
        ...
        for (rk: int32, 0, 64) {
          let cse_var_6: int32 = (ii.c*64)
          let cse_var_5: int32 = (cse_var_6 + jj.c)
          C.wmma.accumulator_1[cse_var_5] = (C.wmma.accumulator_1[cse_var_5] + (cast(float32, A.shared.wmma.matrix_a_1[(cse_var_6 + rk)])*cast(float32, B.shared.wmma.matrix_b_1[((jj.c*64) + rk)])))
        }
      }
    }
    for (ax0_4: int32, 0, 64) {
      for (ax1_4: int32, 0, 64) {
        let cse_var_7: int32 = ((ax0_4*64) + ax1_4)
        C.wmma.accumulator.shared_1: Buffer(C.wmma.accumulator.shared, float32, [4096], [], scope="shared")[cse_var_7] = C.wmma.accumulator_1[cse_var_7]
      }
    }
    for (ii.outer: int32, 0, 4) {
      for (ii.inner: int32, 0, 16) {
        for (jj.outer: int32, 0, 4) {
          for (jj.inner: int32, 0, 16) {
            let cse_var_8: int32 = ((((ii.outer*1024) + (ii.inner*64)) + (jj.outer*16)) + jj.inner)
            C[cse_var_8] = C.wmma.accumulator.shared_1[cse_var_8]
          }
        }
      }
    }
