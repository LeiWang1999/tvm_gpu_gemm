...
log_path = "progress/amos_with_tensorir_avoid_g2g/1.mma_float16_float16_nt"
count = 0
...
# 128x32
def A_permutation(i, j):
    kernel_i, kernel_j = A_global_16x16_to_shared_load_16x16_layout(i % wmma_m, j % wmma_k)
    row = (i // wmma_m) * 16 + kernel_i
    col = (j // wmma_k) * 16 + kernel_j
    # *A_global_16x16_to_shared_load_16x16_layout(i % wmma_m, j % wmma_k))
    return row, col

def B_permutation(i, j, kernel_i, kernel_j):
    return i, j, *shared_load_16x16_to_A_global_16x16_layout(
        kernel_i, kernel_j)


sch.transform_layout(block_tricky_shared_A, ("write", 0),
                     B_permutation, rewrite_type=2)
sch.transform_layout(block_tricky_shared_B, ("write", 0),
                     B_permutation, rewrite_type=2)
